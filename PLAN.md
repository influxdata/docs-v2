...
8. **Examples write to InfluxDB 3.**
   Configuration examples use the `outputs.influxdb_v3` plugin, not `outputs.influxdb_v2`.
   This applies to examples only, not to the InfluxDB v2 output plugin's own documentation or to content where v2 is the subject.
...
Initial set (nav order, weights 101+):

- Monitor system metrics (cpu, mem, disk, net → influxdb\_v3)
...
- Parse log files (tail plus grok patterns)
- Collect metrics from Kafka with JSON parsing
- Collect data from MQTT (topic parsing plus JSON payloads)
- Collect industrial data from OPC UA (IIoT; points to modbus as the alternative)
- Monitor SNMP devices (IF-MIB names first, numeric-OID fallback)
...
| 11 | `docs/telegraf-reference-cleanup`  | Commands, glossary, platforms, release cadence; slim `configuration/_index.md`; light `enterprise.md` refresh; sweep earlier parcels for semicolon and colon sentence joins, menu-level weights, `outputs.influxdb_v2` examples (known: `configuration/_index.md`, `file.md`, `environment-variables.md`, `secrets.md`, `plugin-options.md`), and unlinked first mentions of Telegraf Controller (first mention per page links to `/telegraf/controller/`) | 3, 4, 5    |
