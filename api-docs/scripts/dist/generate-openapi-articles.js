...
function getCleanupPaths(productKey, config) {
    ...
    const articleDataDir = path.join(DOCS_ROOT, 'data/article_data', contentRelPath);
        ...
        for (const article of data.articles) {
            ...
            if (existing) {
                ...
                if (article.fields.related && article.fields.related.length > 0) {
                    ...
                    const existingHrefs = new Set(existingRelated.map((r) => (typeof r === 'string' ? r : r.href)));
...
const productConfigs = {
    ...
    influxdb3_cloud: {
        specFile: path.join(API_DOCS_ROOT, 'influxdb3/cloud/influxdb3-cloud-openapi.yaml'),
...
function deriveProductPath(specPath) {
    ...
    const match = specPath.match(/api-docs\/(?:_build\/)?(enterprise_influxdb|influxdb3?)\/([\w-]+)\//);
    if (!match) {
 ...
 */
function processSpecFile(specConfig, staticPath, staticDirName, contentRelPath) {
    const yaml = require('js-yaml');
    ...
    const articlesPath = path.join(DOCS_ROOT, 'data/article_data', contentRelPath, specSlug);
    try {
...
function processProduct(productKey, config) {
    ...
    const mergedArticlesPath = path.join(DOCS_ROOT, 'data/article_data', contentRelPath, 'api');
    ...
    try {
        ...
        for (const specConfig of specFiles) {
            ...
            const result = processSpecFile(specConfig, staticPath, staticDirName, contentRelPath);
            if (result) {
                ...
                if (!staticOnly && config.useTagBasedGeneration) {
                    ...
                    const pathSpecFiles = openapiPathsToHugo.generatePathSpecificSpecs(result.hugoSpecPath, specPathsPath);
        ...
        if (!staticOnly) {
            if (processedSpecs.length > 1 && config.useTagBasedGeneration) {
                ...
                if (sourceArticles !== targetArticles &&
