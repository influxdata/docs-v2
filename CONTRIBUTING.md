...
docs-v2 automatically runs style linting and code tests for staged files that you try to commit.

For the linting and tests to run, you need to install dependencies.

### Install Git hook and linting dependencies

`package.json` contains dependencies for linting and running Git hooks

- **[Vale](https://vale.sh/)**: lints content files for spelling and style guidelines.
...
1. Install [Yarn](https://yarnpkg.com/getting-started/install) for your system.
2. Run `yarn` to install dependencies (including Hugo and the Vale linter).
3. Install the Yarn package manager and run `yarn` to install project dependencies.

### Install Docker for tests

docs-v2 tests code samples (Shell, Bash, and Python codeblocks) in Markdown files.
`test.sh` starts a Docker container, copies changed files into the container, and runs Pytest
against the codeblocks.

For tests to run, you need to [install and run Docker](https://docs.docker.com/get-docker/) for your system.
...
docs-v2 includes configuration and scripts for linting and testing documentation changes.
...
### Run the linter to find warnings and suggestions

Currently, the pre-commit hook reports error-level alerts from the linter.
To run more exhaustive checks (suggestions, warnings, and errors), run the linter manually using one of the following:

- Run `npx vale`
- Run the custom `.ci/valevale.sh` script that lints changed files using InfluxDB product-specific rules:
