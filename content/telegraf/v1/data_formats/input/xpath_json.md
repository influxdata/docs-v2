...
description: Use the `xpath_json` input data format and XPath expressions to parse JSON into Telegraf metrics.
menu:
  telegraf_v1_ref:
    name: XPath JSON
    weight: 10
...
**int64**. The conversion fails in case the query result is not convertible.
...
**NOTE: Path conversion functions always succeed even if you convert a text
to float.**
