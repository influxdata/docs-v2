---
description: "Telegraf plugin for sending metrics to Zerobus"
menu:
  telegraf_v1_ref:
    parent: output_plugins_reference
    name: Zerobus
    identifier: output-zerobus
tags: [Zerobus, "output-plugins", "configuration", "cloud", "datastore"]
introduced: "v1.40.0"
...
This plugin writes metrics to a [Unity Catalog Delta table](https://docs.databricks.com/aws/en/tables/managed) using the
[Databricks Zerobus Ingest](https://docs.databricks.com/aws/en/ingestion/zerobus-ingest) service. Tags and fields are mapped onto
the columns of the destination table.

> [!IMPORTANT]
> Be aware that this plugin accesses APIs that are [chargeable](https://www.databricks.com/product/pricing/lakeflow-connect) and
> might incur costs.

**Introduced in:** Telegraf v1.40.0
...
`USE SCHEMA`, `SELECT` and `MODIFY` [privileges](https://docs.databricks.com/aws/en/data-governance/unity-catalog/manage-privileges/privileges) on the destination
...
per-request limits and Zerobus's [record size](https://docs.databricks.com/aws/en/ingestion/zerobus-quotas) limit. The write
