...
menu:
  telegraf_v1_ref:
    ...
    name: Systemd
    identifier: secretstore-systemd
tags: [Systemd, "secret-store-plugins", "configuration", "system"]
introduced: "v1.29.0"
...
service by [systemd](https://www.freedesktop.org/wiki/Software/systemd/). Systemd ensures that only the intended service
...
(see [this article](https://systemd.io/CREDENTIALS) for details).

This plugin does not support setting the credentials. See the
credentials management section below for how to
...
[systemd.exec man-page](https://www.freedesktop.org/software/systemd/man/systemd.exec.html) for details.
...
Most steps here are condensed from the [systemd-creds man-page](https://www.freedesktop.org/software/systemd/man/systemd-creds.html).
...
by utilizing TPM2 sealing. See the [systemd-creds man-page](https://www.freedesktop.org/software/systemd/man/systemd-creds.html) for
details.
