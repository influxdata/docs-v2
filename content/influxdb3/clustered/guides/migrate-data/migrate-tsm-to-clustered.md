...
description: >
  ...
  the queried data to an InfluxDB 3 database in your InfluxDB cluster.
menu:
  influxdb3_clustered:
...
alt_links:
  ...
  cloud-serverless: /influxdb3/cloud-serverless/guides/migrate-data/migrate-tsm-to-serverless/
---
...
groups of fields. Call the new custom function from within the existing `data()` function.
For example:
...
1.  **In the {{< product-name omit=" Clustered" >}} cluster you're migrating data _to_**:
    ...
    1. [Create a database](/influxdb3/clustered/admin/databases/create/)
        **to migrate data to**.
    2. [Create a database token](/influxdb3/clustered/admin/tokens/database/create/)
...
2.  **In the InfluxDB Cloud (TSM) organization you're migrating data _from_**:
    ...
    1.  Add the **{{< product-name >}} token (created in step 1b)_**
        as a secret using the key, `influxdb3_clustered_TOKEN`.
...
If the API token you add as the `influxdb3_clustered_TOKEN` doesn't have write
...
- Generate a new {{< product-name >}} token with write access to the database you
  want to migrate to. Then, update the `influxdb3_clustered_TOKEN` secret in your
