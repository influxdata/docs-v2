...
See [`InfluxDBClient3.query()` examples](/influxdb3/clustered/reference/client-libraries/v3/python/#influxdbclient3query) for retrieving the schema.
...
- Non-POSIX-compliant systems (such as Windows) need to specify the root certificates in SslCredentialsOptions for the gRPC client, since the defaults are only configured for POSIX filesystems.
  [Specify the root certificate path](/influxdb3/clustered/reference/client-libraries/v3/python/#specify-the-root-certificate-path) for the Flight gRPC client.
