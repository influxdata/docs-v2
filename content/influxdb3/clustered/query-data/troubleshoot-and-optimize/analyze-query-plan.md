...
Learn how to read and analyze a [query plan](/influxdb3/clustered/reference/glossary/#query-plan) to
...
you can then take steps to [optimize your query](/influxdb3/clustered/query-data/troubleshoot-and-optimize/optimize-queries/), such as add filters to read less data or
...
Use the `EXPLAIN` keyword (and the optional [`ANALYZE`](/influxdb3/clustered/reference/sql/explain/#explain-analyze) and [`VERBOSE`](/influxdb3/clustered/reference/sql/explain/#explain-analyze-verbose) keywords) to view the query plans for a query.
...
- {{% code-placeholder-key %}}`TOKEN`{{% /code-placeholder-key %}}:
  a [database token](/influxdb3/clustered/admin/tokens/#database-tokens) with
...
- one row for the [logical plan](/influxdb3/clustered/reference/internals/query-plan/#logical-plan) (`logical_plan`)
- one row for the [physical plan](/influxdb3/clustered/reference/internals/query-plan/#physical-plan) (`physical_plan`)
...
In a [physical plan](/influxdb3/clustered/reference/internals/query-plan/#physical-plan), each step is an [`ExecutionPlan` node](/influxdb3/clustered/reference/internals/query-plan/#executionplan-nodes) that receives expressions for input data and output requirements, and computes a partition of data.
...
2. Understand the job of each [`ExecutionPlan` node](/influxdb3/clustered/reference/internals/query-plan/#executionplan-nodes)--for example, a [`UnionExec`](/influxdb3/clustered/reference/internals/query-plan/#unionexec) node encompassing the leaf nodes means that the `UnionExec` concatenates the output of all the leaves.
...
The example includes DataFusion and InfluxDB-specific [`ExecutionPlan` nodes](/influxdb3/clustered/reference/internals/query-plans/#executionplan-nodes).
...
- [`ParquetExec`](/influxdb3/clustered/reference/internals/query-plan/#parquetexec) nodes retrieve and scan data from Parquet files in the [Object store](/influxdb3/clustered/reference/internals/storage-engine/#object-store)
- a [`RecordBatchesExec`](/influxdb3/clustered/reference/internals/query-plan/#recordbatchesexec) node retrieves recently written, yet-to-be-persisted data from the [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester)
...
[`RecordBatchesExec`](/influxdb3/clustered/reference/internals/query-plan/#recordbatchesexec) is an InfluxDB-specific `ExecutionPlan` implementation that retrieves recently written, yet-to-be-persisted data from the [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester).

In the example, `RecordBatchesExec` contains the following expressions:

##### `chunks`

`chunks` is the number of data chunks received from the [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester).
...
Neither ParquetExec_A nor ParquetExec_B contain a similar node because data in the Object store is already sorted (by the [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester) or the [Compactor](/influxdb3/clustered/reference/internals/storage-engine/#compactor)) in the given order; the query plan only needs to sort data that arrives from the [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester).
...
A `DeduplicateExec` node indicates that encompassed nodes have [_overlapped data_](/influxdb3/clustered/reference/internals/query-plan/#overlapping-data-and-deduplication)--data in a file or batch have timestamps in the same range as data in another file or batch.
...
The following [sample data](#sample-data) excerpt shows overlapping data between a file and [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester) data:
...
A leaf node that doesn't have a `DeduplicateExec` node in its branch doesn't require deduplication and doesn't overlap other files or [Ingester](/influxdb3/clustered/reference/internals/storage-engine/#ingester) data--for example, ParquetExec_A has no overlaps:
...
To view runtime metrics, such as execution time for a plan and its operators, use [`EXPLAIN ANALYZE`](/influxdb3/clustered/reference/sql/explain/#explain-analyze) to generate the report and use [tracing](/influxdb3/clustered/query-data/troubleshoot-and-optimize/optimize-queries/#enable-trace-logging) for further debugging, if necessary.
