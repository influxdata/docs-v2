...
- {{< req "\*">}} `SELECT`: Specify fields, tags, and calculations to return
  from a [table](/influxdb3/clustered/reference/glossary/#table) or use the
  wildcard alias (`*`) to select all fields and tags from a table. It requires
  at least one
  [field key](/influxdb3/clustered/reference/glossary/#field-key) or the
  wildcard alias (`*`). For more information, see
  [Notable SELECT statement behaviors](/influxdb3/clustered/reference/influxql/select/#notable-select-statement-behaviors).
- {{< req "\*">}} `FROM`: Specify the
  [table](/influxdb3/clustered/reference/glossary/#table) to query from.
...
- `WHERE`: Filter data based on
  [field values](/influxdb3/clustered/reference/glossary/#field),
  [tag values](/influxdb3/clustered/reference/glossary/#tag), or
  [timestamps](/influxdb3/clustered/reference/glossary/#timestamp). Only
...
- An `iox::measurement` column that contains the record's
  [table](/influxdb3/clustered/reference/glossary/#table) name
...
[default time range](/influxdb3/clustered/reference/influxql/group-by/#default-time-range).
...
> [write the sample data](/influxdb3/clustered/reference/sample-data/#write-the-home-sensor-data-to-influxdb)
...
- Specify the [table](/influxdb3/clustered/reference/glossary/#table) to
...
- In the `WHERE` clause, include predicates that compare the tag identifier to a
  string literal. Use
  [logical operators](/influxdb3/clustered/reference/influxql/where/#logical-operators)
...
> [identifier](/influxdb3/clustered/reference/influxql/#identifiers). When
> [aliasing columns in **SQL**](/influxdb3/clustered/query-data/sql/basic-query/#alias-queried-fields-and-tags),
