...
influxdb3/clustered/tags: [Flight client, Java, gRPC, SQL, Flight SQL]
weight: 201
related:
  - /influxdb3/clustered/reference/client-libraries/v3/java/
aliases:
  - /influxdb3/clustered/reference/client-libraries/flight-sql/java-flightsql/
list_code_example: |
    ```java
    public class Query {
...
If you don't already have a database token and a database, see how to [set up InfluxDB](/influxdb3/clustered/get-started/setup/).
If you don't already have data to query, see how to
[write data](/influxdb3/clustered/get-started/write/) to a database.
...
3. In your terminal, run the `docker build` command and pass `--build-arg` flags for the server credentials:
    ...
    - **`DATABASE_NAME`**: your [{{% product-name %}} database](/influxdb3/clustered/admin/databases/)
    - **`HOST`**: your [{{% product-name omit=" Clustered" %}} cluster hostname
      (URL without the "https://")
    - **`TOKEN`**: your [{{% product-name %}} database token](/influxdb3/clustered/get-started/setup/) with _read_ permission to the database
