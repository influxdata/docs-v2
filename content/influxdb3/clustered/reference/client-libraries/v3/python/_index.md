...
influxdb3/clustered/tags: [Flight API, python, gRPC, SQL, client libraries]
metadata: [influxdb3-python v0.10.0]
weight: 201
aliases:
  - /influxdb3/clustered/reference/client-libraries/v3/pyinflux3/
related:
...
> [!Note]
> Code samples in this page use the [Get started home sensor sample data](/influxdb3/clustered/reference/sample-data/#get-started-home-sensor-data).
...
- **`**kwargs`\*\*: Additional write options--for example:
  - **`write_precision`** (string): _Optional_. Default is `"ns"`.
    Specifies the [precision](/influxdb3/clustered/reference/glossary/#precision) (`"ms"`, `"s"`, `"us"`, `"ns"`) for timestamps in `record`.
...
- **time**: the [timestamp](/influxdb3/clustered/reference/glossary/#timestamp) for the record
