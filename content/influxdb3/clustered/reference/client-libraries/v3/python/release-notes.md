---
title: influxdb3-python release notes
list_title: Release notes
description: Release notes for the influxdb3-python client library for InfluxDB 3.
menu:
  influxdb3_clustered:
    name: Release notes
    parent: influxdb3-python
    identifier: influxdb3-python-release-notes
weight: 301
canonical: /influxdb3/core/reference/client-libraries/v3/python/release-notes/
source: /shared/influxdb-client-libraries-reference/v3/release-notes/python.md
---

<!-- Content sourced from content/shared/influxdb-client-libraries-reference/v3/release-notes/python.md -->
