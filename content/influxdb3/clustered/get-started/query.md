...
- Database token using the `--token` flag (use the `INFLUX_TOKEN` environment variable created in
  [Get started--Set up {{< product-name >}}](/influxdb3/clustered/get-started/setup/#configure-authentication-credentials))
- SQL query
...
3.  Install the CLI package (already installed in the [Write data section](/influxdb3/clustered/get-started/write/?t=Python#write-line-protocol-to-influxdb)).
...
4.  Create the `config.json` configuration.
    ...
    - **`DATABASE_TOKEN`**: a [database token](/influxdb3/clustered/admin/tokens/#database-tokens) with
...
1.  Open a terminal in the `influxdb_py_client` module directory you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=Python#write-line-protocol-to-influxdb):
    ...
    3.  Install the following dependencies:
        ...
        {{< req type="key" text="Already installed in the [Write data section](/influxdb3/clustered/get-started/write/?t=Python#write-line-protocol-to-influxdb)" color="magenta" >}}
...
{{< /expand-wrapper >}}
  ...
  2.  Calls the `InfluxDBClient3()` constructor method with credentials to instantiate an InfluxDB `client` with the following credentials:
      ...
      - **`token`**: a [database token](/influxdb3/clustered/admin/tokens/#database-tokens) with
...
1.  In the `influxdb_go_client` directory you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=Go#write-line-protocol-to-influxdb),
...
2.  In `query.go`, enter the following sample code:
    ...
    2.  Defines a `Query()` function that does the following:
        ...
        1.  Instantiates `influx.Client` with the following parameters for InfluxDB credentials:
            ...
            - **`Token`**:  a [database token](/influxdb3/clustered/admin/tokens/#database-tokens) with read permission on the specified database.
              _Store this in a secret store or environment variable to avoid exposing the raw token string._
        ...
        5.  Iterates over rows, formats the timestamp as an [RFC3339 timestamp](/influxdb3/clustered/reference/glossary/#rfc3339-timestamp), and prints the data in table format to stdout.

3.  In your editor, open the `main.go` file you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=Go#write-line-protocol-to-influxdb) and insert code to call the `Query()` function--for example:
...
_This tutorial assumes you installed Node.js and npm, and created an `influxdb_js_client` npm project as described in the [Write data section](/influxdb3/clustered/get-started/write/?t=Nodejs)._

1.  In your terminal or editor, change to the `influxdb_js_client` directory you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=Nodejs).
...
4.  Inside of `query.mjs`, enter the following sample code:
    ...
    */
    ...
    2.  Calls `new InfluxDBClient()` and passes a `ClientOptions` object to instantiate a client configured
        ...
        - **`host`**: your {{% product-name omit=" Clustered" %}} cluster URL
        - **`token`**: a [database token](/influxdb3/clustered/admin/tokens/#database-tokens) with read permission on the database you want to query.
          _Store this in a secret store or environment variable to avoid exposing the raw token string._
...
5.  Inside of `index.mjs` (created in the [Write data section](/influxdb3/clustered/get-started/write/?t=Nodejs)), enter the following sample code to import the modules and call the functions:
...
1.  In the `influxdb_csharp_client` directory you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=C%23),
...
3.  In your editor, open the `Program.cs` file you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=C%23#write-line-protocol-to-influxdb) and insert code to call the `Query()` function--for example:
...
_This tutorial assumes using Maven version 3.9, Java version >= 15, and an `influxdb_java_client` Maven project created in the [Write data section](/influxdb3/clustered/get-started/write/?t=Java)._

1.  In your terminal or editor, change to the `influxdb_java_client` directory you created in the
    [Write data section](/influxdb3/clustered/get-started/write/?t=Java).
...
{{< page-nav prev="/influxdb3/clustered/get-started/write/" keepTab=true >}}
