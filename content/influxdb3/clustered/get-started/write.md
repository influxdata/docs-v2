...
information, see the
[Line protocol reference](/influxdb3/clustered/reference/syntax/line-protocol/)._
...
- {{< req "\*" >}} **measurement**: A string that identifies the
  [table](/influxdb3/clustered/reference/glossary/#table) to store the data in.
...
- {{< req "\*" >}} **field set**: Comma-delimited list of key value pairs, each
  ...
  Field values can be [strings](/influxdb3/clustered/reference/syntax/line-protocol/#string)
  (quoted),
  ...
  or [booleans](/influxdb3/clustered/reference/syntax/line-protocol/#boolean).
...
[InfluxDB schema design](/influxdb3/clustered/write-data/best-practices/schema-design/)._
...
To learn more about available tools and options, see [Write data](/influxdb3/clustered/write-data/).
...
Use the [`influxctl write` command](/influxdb3/clustered/reference/cli/influxctl/write/)
...
request to the
[InfluxDB API `/write` endpoint](/influxdb3/clustered/api/data-api/write-data/)
using the `POST` request method.

{{% api-endpoint endpoint="https://{{< influxdb/host >}}/write" method="post" api-ref="/influxdb3/clustered/api/data-api/write-data/"%}}
...
- **Query parameters**:
  ...
  - **precision**:[timestamp precision](/influxdb3/clustered/reference/glossary/#timestamp-precision) (default is `ns`)
- **Request body**: Line protocol as plain text

> [!Note]
> With the {{% product-name %}}
> [v1 API `/write` endpoint](/influxdb3/clustered/api/data-api/write-data/),
...
To write data to InfluxDB using the
[InfluxDB v2 HTTP API](/influxdb3/clustered/reference/api/), send a request
...
method="post" api-ref="/influxdb3/clustered/api/data-api/write-data/" >}}
...
> schemes, see how to
> [authenticate API requests](/influxdb3/clustered/guides/api-compatibility/v2/).
...
6. Inside of `write.py`, enter the following sample code:
   ...
   4. Calls the `client.write()` method with the line protocol record list and
      ...
      **Because the timestamps in the sample line protocol are in second
      ...
      [timestamp precision](/influxdb3/clustered/reference/glossary/#timestamp-precision)
      to seconds.**
...
5.  In your editor, create a `Write.cs` file and enter the following sample
    ...
    3.  Calls the client's `WriteRecordAsync()` method to write each line
        ...
        **Because the timestamps in the sample line protocol are in second
        ...
        the[timestamp precision](/influxdb3/clustered/reference/glossary/#timestamp-precision)
        to seconds.**
...
{{< page-nav prev="/influxdb3/clustered/get-started/setup/"
next="/influxdb3/clustered/get-started/query/" keepTab=true >}}
