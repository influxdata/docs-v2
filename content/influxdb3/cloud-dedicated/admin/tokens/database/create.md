...
2.  In your terminal, use cURL to send a request to the following {{% product-name %}} endpoint:
    ...
    {{% api-endpoint endpoint="https://console.influxdata.com/api/v0/accounts/ACCOUNT_ID/clusters/CLUSTER_ID/tokens" method="post" api-ref="/influxdb3/cloud-dedicated/api/database-tokens/#operation/CreateDatabaseToken" %}}
...
For [token details](/influxdb3/cloud-dedicated/api/database-tokens/#operation/CreateDatabaseToken) and easier programmatic access to the command output, include `--format json`
