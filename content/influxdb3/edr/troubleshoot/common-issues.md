---
title: Troubleshoot common EDR issues
description: >
...
- **Replication stopped ("halted")**—run `edr-inspect metrics` (live), or
  ...
  you the halt **class** (auth, schema, or network). Fix the
  cause—the halted batch retries as a probe and **auto-resumes**;
  nothing is skipped.
- **Lag climbing ("behind")**—run `edr-inspect state`, then
  ...
  **lost** (rare) or just **owed backlog** that drains once the link
  recovers. Lag != loss.
- **Data missing**—run `edr-inspect state` (lost and gap counts).
  **The only authority on loss.** A non-zero `lost(unrecoverable)`
  here is real; a scary `metrics` reading is not.
- **Agent crashed or restarted**—capture the log, then follow the
  log post-mortem procedure below. In-memory metrics reset on
  restart—the log is the only witness.

> [!Important]
> Trust order when the evidence disagrees: durable `state` > the log's
> sequence > a single live `metrics` snapshot. A `metrics` or UI
> reading is point-in-time and resets each run, so it can catch a
> transient at its worst instant. Always confirm a conclusion against
> `edr-inspect state` before acting—see
> [Data not flowing](#data-not-flowing).
...
- **"downstream.historic_fill must be set explicitly ..."**—no historic
  intent declared. Add `historic_fill: { mode: none }` (live-only),
  `full`, or `since`. See
  [Historic fill](/influxdb3/edr/monitor/historic-and-gap-fill/#historic-fill).
- **"historic_fill mode 'since' requires a 'since' value", or "... is
  not parseable"**—`since` missing or malformed. Set `since` to a
  whole-day duration (`7d`) or an ISO date (`2026-06-01`); sub-day
  units (`30m` or `6h`) are rejected.
...
1. First-line triage: `edr-inspect state <STATE_LOCATION>` (what's owed
   and what's in progress, even if the agent is wedged) and
   `edr-inspect topology` and `edr-inspect metrics` (live health and
   flow). See
...
3. Check connectivity: `curl http://destination:9090/health` (network
   listener)
4. Check [write-ahead log (WAL)](/influxdb3/edr/reference/glossary/#wal-write-ahead-log)
   files exist: `ls <DATA_DIR>/<NODE_ID>/pt_wal/`
5. Check scope includes the database and table.
...
- **`historic fill: MANIFEST GAP ...`**—expected: the compactor deleted
  old manifests; recovery falls back to WAL files, then to
...
- **`... POTENTIAL DATA LOSS`**—not expected: data was deleted from every
  tier (WAL,
  [gen0](/influxdb3/enterprise/reference/storage-engine-config-options/#gen0),
  cv2) before EDR could read it. Investigate retention settings. Historic
  fill emits this as `historic fill: MANIFEST GAP —
  ... no cv2 files found covering the gap — POTENTIAL DATA LOSS`.
- **Schema conflict, or channel blocked, retrying every 60s**—destination
...
- **Cursor reset detected**—InfluxDB WAL IDs reset (for example, after
  ...
  its cursor.
- **`failed to connect to downstream` followed by a burst of
  `batch send failed`, right after a restart**—restart-ordering gap:
  the source came up before the destination's listener was accepting.
  Clears at the next `connected to downstream` or `edge connected`.
  Benign and self-healing; not data loss.
...
- Agents released before protocol versioning (0.1.x and 0.2.x) are
  always accepted as protocol v1—they can never be refused by current
  builds.
...
- **Lag is not loss.** A high `live-lag` or `pending_bytes` is almost
  always **owed backlog** that drains once the destination catches up,
  not missing data. `edr-inspect state` (`lost(unrecoverable)`, pending
  gaps) is the authority on what—if anything—is actually unrecoverable.
  Confirm there before treating lag as an incident.
...
- Slow WAL discovery -> lower `--poll-interval-ms` (more frequent object
  store listing).
- Constrained link -> check whether a `bandwidth_schedule` window is
  active (`limited` or `silent` shows in the UI and logs).
...
- If the buildup of already-replicated WAL is a problem, enable EDR's
  ...
  or backfill.
- After enabling, confirm `gaps_pending` and `wal_files_lost_total`
  stay flat—a rise is not expected (cleanup is designed never to
  create gaps) and warrants a look. A sweep that prunes anything logs
  `wal cleanup: deleted replicated WAL files`; delete failures warn
  and hold the watermark.
...
3. A `shutdown signal received` followed by a `... shutting down`
   cascade means a clean stop; **no** shutdown markers means an
   unclean exit (crash, out-of-memory (OOM), or kill).
4. Identify the dominant ERROR or WARN class and its fields (`table`,
   `upstream`, `error`).
5. Write a one-line verdict and remediation.

For the complete rule set—delivery failures, data loss, cursor and
state, connectivity, config, and infrastructure signatures—see the
