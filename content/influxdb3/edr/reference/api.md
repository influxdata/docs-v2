...
Each data batch is acknowledged synchronously by the HTTP response. The
...
| `/edr/v1/data` | POST | Data batches (LP or PT+zstd encoded). |
...
| `/edr/v1/node_info` | GET | Identity, role, location. |
...
reason appears in the status API/UI/logs, and a WARN repeats once a minute
