...
list_title: Python
description: The InfluxDB 3 `influxdb3-python` Python client library integrates with Python scripts and applications to write and query data stored in an InfluxDB Cloud Serverless bucket.
menu:
  influxdb3_cloud_serverless:
    name: Python
    parent: v3 client libraries
    identifier: influxdb3-python
influxdb3/cloud-serverless/tags:
  [Flight API, python, gRPC, SQL, client libraries]
metadata: [influxdb3-python v0.10.0]
weight: 201
aliases:
  - /influxdb3/cloud-serverless/reference/client-libraries/v3/pyinflux3/
related:
...
> [!Note]
> Code samples in this page use the [Get started home sensor sample data](/influxdb3/cloud-serverless/reference/sample-data/#get-started-home-sensor-data).
...
```python { placeholders="BUCKET_NAME|API_TOKEN" }
import os
```
-->
...
- {{% code-placeholder-key %}}`BUCKET_NAME`{{% /code-placeholder-key %}}: the name of your {{% product-name %}} [database](/influxdb3/cloud-serverless/admin/buckets/)
- {{% code-placeholder-key %}}`API_TOKEN`{{% /code-placeholder-key %}}:
  an {{% product-name %}} [API token](/influxdb3/cloud-serverless/admin/tokens/)
...
- **`bucket`** (string): _Optional_. The bucket to write to. Default is to write to the bucket specified for the client.
- **`**kwargs`\*\*: Additional write options--for example:
  - **`write_precision` (string)**: _Optional_. Default is `"ns"`.
    Specifies the [precision](/influxdb3/cloud-serverless/reference/glossary/#precision) (`"ms"`, `"s"`, `"us"`, `"ns"`) for timestamps in `record`.
...
- {{% code-placeholder-key %}}`API_TOKEN`{{% /code-placeholder-key %}}:
  an {{% product-name %}} [API token](/influxdb3/cloud-serverless/admin/tokens/)
  with write permissions on the specified bucket
...
- **time**: the [timestamp](/influxdb3/cloud-serverless/reference/glossary/#timestamp) for the record
...
- {{% code-placeholder-key %}}`BUCKET_NAME`{{% /code-placeholder-key %}}:
  the name of your {{% product-name %}} [database](/influxdb3/cloud-serverless/admin/buckets/)
- {{% code-placeholder-key %}}`API_TOKEN`{{% /code-placeholder-key %}}:
  an {{% product-name %}} [API token](/influxdb3/cloud-serverless/admin/tokens/)
  with read permission on the specified database
...
<a class="btn" href="https://github.com/InfluxCommunity/influxdb3-python/" target="\_blank">View the InfluxDB 3 Python client library</a>
