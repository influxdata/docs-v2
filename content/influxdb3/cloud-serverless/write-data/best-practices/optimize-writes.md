...
> 
> - [`influx` CLI](/influxdb3/cloud-serverless/reference/cli/influx/write/)
...
_Specify timestamp precision when [writing to InfluxDB](/influxdb3/cloud-serverless/write-data/)._
...
Each [InfluxDB client library](/influxdb3/cloud-serverless/reference/client-libraries/) provides
...
[InfluxDB client libraries documentation](/influxdb3/cloud-serverless/reference/client-libraries/).
{{% /tab-content %}}
{{% tab-content %}}
...
```bash { placeholders="BUCKET_NAME|API_TOKEN|ORG_NAME" callout="Content-Encoding: gzip" callout-color="orange" }
...
- {{% code-placeholder-key %}}`ORG_NAME`{{% /code-placeholder-key %}}: the name of your [organization](/influxdb3/cloud-serverless/admin/organizations/)
- {{% code-placeholder-key %}}`BUCKET_NAME`{{% /code-placeholder-key %}}: the name of the [bucket](/influxdb3/cloud-serverless/admin/buckets/) to write data to
- {{% code-placeholder-key %}}`DATABASE_TOKEN`{{% /code-placeholder-key %}}: a [token](/influxdb3/cloud-serverless/admin/tokens/) with _write_ access to the specified bucket.
...
Pre-processing data in your write workload can help you avoid [write failures](/influxdb3/cloud-serverless/write-data/troubleshoot/#troubleshoot-failures) due to schema conflicts or resource use.
...
1.  Enter the following command to create a Telegraf configuration that parses system usage data, removes the specified fields and tags, and then writes the data to InfluxDB:
    ...
    - {{% code-placeholder-key %}}`ORG_NAME`{{% /code-placeholder-key %}}: the name of your [organization](/influxdb3/cloud-serverless/admin/organizations/)
    - {{% code-placeholder-key %}}`BUCKET_NAME`{{% /code-placeholder-key %}}: the name of the [bucket](/influxdb3/cloud-serverless/admin/buckets/) to write data to
    - {{% code-placeholder-key %}}`API_TOKEN`{{% /code-placeholder-key %}}: a [token](/influxdb3/cloud-serverless/admin/tokens/) with _write_ access to the specified bucket.
      _Store this in a secret store or environment variable to avoid exposing the raw token string._
...
[Get started home sensor data](/influxdb3/cloud-serverless/reference/sample-data/#get-started-home-sensor-data) to a bucket, and then try to write the following batch to the same measurement:
...
The following example converts the `temp`, `hum`, and `co` fields to fit the [sample data](/influxdb3/cloud-serverless/reference/sample-data/#get-started-home-sensor-data) schema:
