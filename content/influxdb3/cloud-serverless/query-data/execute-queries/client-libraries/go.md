...
influxdb3/cloud-serverless/tags: [Flight client, query, flight, go, sql, influxql]
related:
...
The following examples show how to create an [InfluxDB client](/influxdb3/cloud-serverless/reference/client-libraries/v3/go/#function-new), use client query methods to select all fields in a measurement, and then access query result data and metadata.
...
- {{% code-placeholder-key %}}`API_TOKEN`{{% /code-placeholder-key %}}: an InfluxDB [API token](/influxdb3/cloud-serverless/admin/tokens/) with _read_ permission on the specified bucket
