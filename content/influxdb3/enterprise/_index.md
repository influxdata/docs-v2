---
title: InfluxDB 3 Enterprise Guide
description: >
...
This guide covers not only Enterprise, but also everything in InfluxDB 3 Core (the open source release) Core's feature highlights include:
...
If you are looking to get started quickly with an installation on your local machine, we recommend using our install script below. Regardless of your platform OS, it will handle the download and installation of InfluxDB 3 Enterprise. If you just want to find the built artifacts or Docker images and install them yourself, details are after the install script instructions.
...
If you're starting InfluxDB 3 Enterprise for the first time, you'll be asked to enter an email address for verification. You'll be sent an email with a link to click to verify you received the email. Upon verification, the license creation, retrieval, and application is automated. During the alpha period you'll be granted a license that lasts until May 7th, 2025.
...
InfluxDB 3 Enterprise supports a **last-n values cache** which stores the last N values in a series or or column heirarchy in memory. This gives the database the ability to answer these kinds of queries in under 10 milliseconds. The CLI can be used to create a last value cache, which uses a REST API on the database.
