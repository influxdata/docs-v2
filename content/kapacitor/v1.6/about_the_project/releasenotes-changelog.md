...
- For alert templates, row templates, and details templates, add support for third-party services that reject standard `json` (with carriage return at end) by adding the ability to use compact `json` in templates. To compact, add double curly braces (`{{ }}`) around your JSON payload. (This change also compacts Big Panda alert details to avoid Panda service error.)
