...
The [`kapacitor stats ingress`](/kapacitor/v1.6/working/cli_client/#stats-ingress) command
...
You can also use Kapacitor's [`/debug/vars` API endpoint](/kapacitor/v1.6/working/api/#debug-vars-http-endpoint)
...
If data is ingested at irregular intervals and you see unexpected results with the same timestamp, use the [`log node`](/kapacitor/v1.6/nodes/log_node) when ingesting data in your TICKscript to debug issues. This surfaces issues, for example, duplicate data hidden by httpOut.
