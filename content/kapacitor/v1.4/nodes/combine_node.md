---
title: CombineNode (Kapacitor TICKscript node)
...
|:---------|:---------|
| **[combine](#descr)&nbsp;(&nbsp;`expressions`&nbsp;`...ast.LambdaNode`)** | Combine this node with itself. The data is combined on timestamp.  |
...
Combinations are order independent and never include the same point multiple times.
...
In the example above, the data points for the `login` service are combined with the data points from all other services.
...
In the example above, all combinations triples are created.
