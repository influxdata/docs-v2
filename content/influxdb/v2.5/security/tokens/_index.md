...
aliases:
  - /influxdb/v2.6/users/tokens
influxdb/v2.6/tags: [tokens, authentication, security]
menu:
  influxdb_2_5:
    name: Manage tokens
...
Some operations, e.g. [retrieving the server configuration](/influxdb/v2.6/reference/config-options/), require operator permissions.
Operator tokens are created in the InfluxDB setup process.
To [create an operator token manually](/influxdb/v2.6/security/tokens/create-token/) with the InfluxDB UI, `api/v2` API, or `influx` CLI after the setup process is completed, you must use an existing [Operator token](/influxdb/v2.6/security/tokens/#operator-token).
...
we recommend [creating an All-Access token](/influxdb/v2.6/security/tokens/create-token/)
