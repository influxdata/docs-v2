...
description: >
  Customize your InfluxDB configuration by using [`influxd`](/influxdb/v2.5/reference/cli/influxd/)
...
products: [oss]
related:
  - /influxdb/v2.5/reference/cli/influxd
---
...
Server configuration commands require an [Operator token](/influxdb/v2.5/security/tokens/#operator-token).

#### View your server configuration with the CLI

Use the [`influx server-config` command](/influxdb/v2.5/reference/cli/influx/server-config/)
...
[{{< api-endpoint method="GET" endpoint="http://localhost:8086/api/v2/config" >}}]((/influxdb/v2.5/api/#operation/GetConfig))
...
[`influxd` service](/influxdb/v2.5/reference/cli/influxd):
...
Enable [additional security features](/influxdb/v2.5/security/enable-hardening/)
in InfluxDB.
**Default:** `false`
| influxd flag          | Environment variable        | Configuration key   |
| :-------------------- | :-------------------------- | :------------------ |
...
### metrics-disabled
Disable the HTTP `/metrics` endpoint which exposes [internal InfluxDB metrics](/influxdb/v2.5/reference/internals/metrics/).
...
_For more information, see [Enable TLS encryption](/influxdb/v2.5/security/enable-tls/)._
