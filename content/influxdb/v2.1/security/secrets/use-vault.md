...
using the [`--vault-addr` flag](/influxdb/v2.1/reference/config-options/#vault-addr) when
...
(required to access your Vault server) using the [`--vault-token` flag](/influxdb/v2.1/reference/config-options/#vault-token)
...
Start the [`influxd` service](/influxdb/v2.1/reference/cli/influxd/) with the `--secret-store`
...
For details, see [Manage secrets](/influxdb/v2.1/security/secrets/manage-secrets/).
