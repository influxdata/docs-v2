...
Use the [`influx backup` command](/influxdb/v2.1/reference/cli/influx/backup/) to back up
...
{{% note %}}
#### InfluxDB 1.x/2.1 compatibility
...
- [Automatically upgrade from InfluxDB 1.x to {{< current-version >}}](/influxdb/v2.1/upgrade/v1-to-v2/automatic-upgrade/)
- [Manually upgrade from InfluxDB 1.x to {{< current-version >}}](/influxdb/v2.1/upgrade/v1-to-v2/manual-upgrade/)
{{% /note %}}
...
- The **root authorization token** (the token created for the first user in the
  [InfluxDB setup process](/influxdb/v2.1/get-started/)).
