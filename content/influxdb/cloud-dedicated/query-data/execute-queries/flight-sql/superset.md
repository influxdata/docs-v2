...
3.  In the **Connect a Database** window, click on the **Supported Databases**
    ...
    {{< img-hd src="/img/influxdb/cloud-dedicated-superset-connect.png" alt="Configure InfluxDB connection in Superset" />}}
...
5.  Enter your **SQL Alchemy URI** comprised of the following:
    ...
    **Query parameters**
    ...
    - **`?database-name`**: URL-encoded InfluxDB [database name](influxdb/cloud-dedicated/admin/databases/list/)
...
```sh
# Syntax
datafusion+flightsql://<domain>:<port>?database-name=<database-name>&token=<token>

# Example
datafusion+flightsql://cluster-id.influxdb.io:443?database-name=example-database&token=example-token
```
{{< /code-callout >}}
    {{< /code-callout >}}
...
2.  In the left pane:
    ...
    {{< img-hd src="/img/influxdb/cloud-dedicated-superset-schema.png" alt="Select your InfluxDB schema in Superset" />}}
