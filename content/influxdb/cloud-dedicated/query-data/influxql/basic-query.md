...
- {{< req "\*">}} `SELECT`: Specify fields, tags, and calculations to return from a
  [table](/influxdb/cloud-dedicated/reference/glossary/#table) or use the wildcard alias (`*`) to select all fields and tags
...
- {{< req "\*">}} `FROM`: Specify the [table](/influxdb/cloud-dedicated/reference/glossary/#table) to query from.
...
- An `iox::measurement` column that contains the record's [table](/influxdb/cloud-dedicated/reference/glossary/#table) name
...
- Specify the [table](/influxdb/cloud-dedicated/reference/glossary/#table) to query in the `FROM` clause.
