...
The data consist of the [measurement](/influxdb/v1.6/concepts/glossary/#measurement) `cpu_load_short`, the [tag keys](/influxdb/v1.6/concepts/glossary/#tag-key) `host` and `region` with the [tag values](/influxdb/v1.6/concepts/glossary/#tag-value) `server01` and `us-west`, the [field key](/influxdb/v1.6/concepts/glossary/#field-key) `value` with a [field value](/influxdb/v1.6/concepts/glossary/#field-value) of `0.64`, and the [timestamp](/influxdb/v1.6/concepts/glossary/#timestamp) `1434055562000000000`.
<br>
...
See the [API Reference](/influxdb/v1.6/tools/api/#write-http-endpoint) documentation for a complete list of the available query parameters.

The body of the POST - we call this the [Line Protocol](/influxdb/v1.6/concepts/glossary/#line-protocol) - contains the time-series data that you wish to store.
...
Field keys are required and are always strings, and, [by default](/influxdb/v1.6/write_protocols/line_protocol_reference/#data-types), field values are floats.
...
This simple, easy to understand system uses HTTP as a transfer method for [InfluxQL](/influxdb/v1.6/query_language/spec/).
...
### Next steps
---
Now that you know how to write data with the built-in HTTP API discover how to query them with the [Querying Data](/influxdb/v1.6/guides/querying_data/) guide!
For more information about writing data with the HTTP API, please see the [API reference documentation](/influxdb/v1.6/tools/api/#write-http-endpoint).
