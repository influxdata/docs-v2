...
## The filter() function
`filter()` has an `fn` parameter that expects a [predicate function](/influxdb/cloud/reference/glossary/#predicate-function),
an anonymous function comprised of one or more [predicate expressions](/influxdb/cloud/reference/glossary/#predicate-expression).
...
Use [logical operators](/influxdb/cloud/reference/flux/language/operators/#logical-operators)
...
The combination of [`from()`](/influxdb/cloud/reference/flux/stdlib/built-in/inputs/from),
[`range()`](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/range),
and `filter()` represent the most basic Flux query:

1. Use `from()` to define your [bucket](/influxdb/cloud/reference/glossary/#bucket).
