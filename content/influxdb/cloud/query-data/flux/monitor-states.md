...
weight: 220
aliases:
  - /influxdb/cloud/query-data/guides/monitor-states/
related:
...
- [Get started with Flux](/influxdb/cloud/query-data/get-started/) for a conceptual overview of Flux.
...
1. Use the [`stateDuration()`](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/stateduration/) function to calculate how long a column value has remained the same value (or state). Include the following information:
...
1. Use the [`stateCount()` function](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/statecount/)
...
Detect state changes with the `monitor.stateChanges()` function. To use the `monitor.stateChanges()` function, set up a **check** to query data (stored in the `_monitoring` bucket > `statuses` measurement > `_level` column; see [Monitor data and send alerts](/influxdb/cloud/monitor-alert/) for more detail.

1. In the InfluxDB user interface, click the **Monitoring and Alerting** icon from the sidebar.

    {{< nav-icon "alerts" >}}

2. If you haven't already, [create a check](/influxdb/cloud/monitor-alert/checks/create/) that stores statuses (`CRIT`, `WARN`, `INFO`, `OK` or `ANY`) in the `_level` column. <!-- specify how to do this with monitor.check() function or in UI, with check threshold or deadman?
