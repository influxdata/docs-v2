...
weight: 220
aliases:
  - /influxdb/cloud/query-data/guides/custom-functions/
list_code_example: |
  ```js
  multByX = (tables=<-, x) =>
    tables
...
It uses the [`map()` function](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/map)
...
It uses the [`sort()` function](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/sort)
to sort records in either descending or ascending order.
It then uses the [`limit()` function](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/limit)
