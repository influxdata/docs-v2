...
alt_links:
  cloud-serverless: /influxdb3/cloud-serverless/admin/buckets/update-bucket/
---
...
or the [InfluxDB HTTP API `PATCH /api/v2/buckets` endpoint](/influxdb/cloud/api/buckets/) to set a custom retention period.
    {{% /note %}}
...
Use the InfluxDB HTTP API [`PATCH /api/v2/buckets` endpoint](/influxdb/cloud/api/buckets/)
to update a bucket.
...
1. To find the bucket ID, send a request to the HTTP API [`GET /api/v2/buckets/` endpoint](/influxdb/cloud/api/buckets/) to retrieve the list of buckets. <!-- @TODO: provide API auth note about tokens and read access to buckets -->

    {{< api-endpoint method="get" endpoint="https://cloud2.influxdata.com/api/v2/buckets" api-ref="/influxdb/cloud/api/buckets/" >}}

2. Send a request to the HTTP API [PATCH `/api/v2/buckets/{BUCKET_ID}` endpoint](/influxdb/cloud/api/buckets/).
