...
- Start the `telegraf` agent using the `--config` flag to provide the URL of the
  InfluxDB Telegraf configuration. For example:
    ...
    ```sh
    telegraf --config https://cloud2.influxdata.com/api/v2/telegrafs/<telegraf-config-id>
    ```

{{% note %}}
_[Setup instructions](/influxdb/cloud/telegraf-configs/view/#view-setup-instructions) for
