...
aliases:
  - /influxdb/cloud/users/tokens/create-token/
menu:
  influxdb_cloud:
    name: Create a token
...
Use the [`influx auth create` command](/influxdb/cloud/reference/cli/influx/auth/create) to create a token.
Include flags with the command to grant specific permissions to the token.
See the [available flags](/influxdb/cloud/reference/cli/influx/auth/create#flags).
...
See the [`influx auth list` documentation](/influxdb/cloud/reference/cli/influx/auth/list)
