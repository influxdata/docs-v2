...
InfluxDB and Flux return query results in annotated CSV format.
You can also read annotated CSV directly from Flux with the [`csv.from()` function](/influxdb/cloud/reference/flux/stdlib/csv/from/)
or write data to InfluxDB using annotated CSV and the `influx write` command.
...
{{% note %}}
To encode a table with its [group key](/influxdb/cloud/reference/glossary/#group-key),
...
[`influx write` command](/influxdb/cloud/reference/cli/influx/write/) to write annotated CSV to InfluxDB.
...
[line protocol data types and format](/influxdb/cloud/reference/syntax/line-protocol/#data-types-and-format)._
...
Time column values should be **Unix timestamps** (in an [accepted timestamp precision](/influxdb/cloud/write-data/#timestamp-precision)),
...
The example below illustrates how to use the [`csv.from()` function](/influxdb/cloud/reference/flux/stdlib/csv/from/)
