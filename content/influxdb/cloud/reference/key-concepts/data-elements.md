...
_For information about tag key requirements, see [Line protocol – Tag set](/influxdb/cloud/reference/syntax/line-protocol/#tag-set)._
...
_For information about tag value requirements, see [Line protocol – Tag set](/influxdb/cloud/reference/syntax/line-protocol/#tag-set)._
...
InfluxDB scans every field value in the dataset for `bees` before the query returns a response. If our sample `census` data grew to millions of rows, to optimize your query, you could rearrange your [schema](/influxdb/cloud/reference/glossary/#schema) so the fields (`bees` and `ants`) becomes tags and the tags (`location` and `scientist`) become fields:
...
Understanding the concept of a series is essential when designing your [schema](/influxdb/cloud/reference/glossary/#schema) and working with your data in InfluxDB.
...
All InfluxDB data is stored in a bucket. A **bucket** combines the concept of a database and a retention period (the duration of time that each data point persists). A bucket belongs to an organization. For more information about buckets, see [Manage buckets](/influxdb/cloud/organizations/buckets/).

## Organization

An InfluxDB **organization** is a workspace for a group of [users](/influxdb/cloud/users/). All [dashboards](/influxdb/cloud/visualize-data/dashboards/), [tasks](/influxdb/cloud/process-data/), buckets, and users belong to an organization. For more information about organizations, see [Manage organizations](/influxdb/cloud/organizations/).

If you're just starting out, we recommend taking a look at the following guides:

- [Get started](/influxdb/cloud/get-started)
