...
- Add option for Cloud users to use the `influx` CLI to interact with a Cloud instance. For more information, see how to [download and install the influx CLI](/influxdb/cloud/get-started/#optional-download-install-and-use-the-influx-cli) and then learn more about how the [influx - InfluxDB command line interface](/influxdb/cloud/reference/cli/influx/) works.
...
- Add option to display dashboards in [light mode](/influxdb/cloud/visualize-data/dashboards/control-dashboard/#toggle-dark-mode-and-light-mode).
- Add [shell `completion` commands](/influxdb/cloud/reference/cli/influx/completion/) to the `influx` CLI.
...
- [Delete bucket by name](/influxdb/cloud/organizations/buckets/delete-bucket/#delete-a-bucket-by-name) using the `influx` CLI.
- Add helper module to write line protocol to specified url, org, and bucket.
- Add [`pkg stack`](/influxdb/cloud/reference/cli/influx/stacks) for stateful package management.
...
- Add an [easy way to switch configurations](/influxdb/cloud/reference/cli/influx/config/#quickly-switch-between-configurations) using the `influx` CLI.
...
- Move to [Flux 0.34.2](/influxdb/cloud/reference/release-notes/flux/#v0-34-2-2019-06-27) -
...
Before upgrading, [export all existing tasks](/influxdb/cloud/process-data/manage-tasks/export-task/). After upgrading, [reimport your exported tasks](/influxdb/cloud/process-data/manage-tasks/create-task/#import-a-task).
{{% /warn %}}
