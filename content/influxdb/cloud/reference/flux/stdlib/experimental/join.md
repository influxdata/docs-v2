...
The `experimental.join()` function joins two streams of tables on the
[group key](/influxdb/cloud/reference/glossary/#group-key) and `_time` column.
Use the [`fn` parameter](#fn) to map new output tables using values from input tables.

{{% note %}}
To join streams of tables with different fields or measurements, use [`group()`](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/group/)
or [`drop()`](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/drop/) to remove
...
{{% note %}}
This function will likely replace the [`join` function](/influxdb/cloud/reference/flux/stdlib/built-in/transformations/join/)
