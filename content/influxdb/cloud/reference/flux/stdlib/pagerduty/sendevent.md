...
aliases:
  - /influxdb/cloud/reference/flux/functions/pagerduty/sendevent/
menu:
  influxdb_cloud_ref:
...
{{% note %}}
When using [`pagerduty.endpoint()`](/influxdb/cloud/reference/flux/stdlib/pagerduty/endpoint/)
to send data to PagerDuty, the function uses the [`pagerduty.dedupKey()` function](/influxdb/cloud/reference/flux/stdlib/pagerduty/dedupkey/) to populate the `dedupkey` parameter.
{{% /note %}}
