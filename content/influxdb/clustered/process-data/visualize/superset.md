...
aliases:
  ...
  - /influxdb/clustered/query-data/sql/execute-queries/superset/
  - /influxdb/clustered/process-data/tools/superset/
related:
  - /influxdb/clustered/visualize-data/superset/
metadata: [SQL only]
---
...
5.  Enter your **SQL Alchemy URI** comprised of the following:
    ...
    **Query parameters**
    ...
    - **`?database`**: URL-encoded InfluxDB [database name](/influxdb/clustered/admin/databases/list/)
    - **`?token`**: InfluxDB [API token](/influxdb/clustered/get-started/setup/) with `READ` access to the specified database
