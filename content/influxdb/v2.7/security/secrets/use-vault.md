...
using the [`--vault-addr` flag](/influxdb/v2.7/reference/config-options/#vault-addr) when
...
(required to access your Vault server) using the [`--vault-token` flag](/influxdb/v2.7/reference/config-options/#vault-token)
...
Start the [`influxd` service](/influxdb/v2.7/reference/cli/influxd/) with the `--secret-store`
...
For more information, see [InfluxDB configuration options](/influxdb/v2.7/reference/config-options/).

## Manage secrets through the InfluxDB API

Use the InfluxDB `/org/{orgID}/secrets` API endpoint to add tokens to Vault.
For details, see [Manage secrets](/influxdb/v2.7/security/secrets/manage-secrets/).
