...
related:
  ...
  - /influxdb/v2.7/reference/prometheus-metrics/
  - /influxdb/v2.7/write-data/no-code/scrape-data/
influxdb/v2.7/tags: [prometheus, scraper]
---

Use [Telegraf](/{{< latest "telegraf" >}}/){{% oss-only %}}, [InfluxDB scrapers](/influxdb/v2.7/write-data/no-code/scrape-data/),{{% /oss-only %}}
...
1. Add the [Prometheus input plugin](/{{< latest "telegraf" >}}/plugins/#input-prometheus) to your Telegraf configuration file.
    ...
    2. Set the `metric_version` configuration option to specify which
      [metric parsing version](/influxdb/v2.7/reference/prometheus-metrics/) to use
...
[Scrape data using InfluxDB scrapers](/influxdb/v2.7/write-data/no-code/scrape-data/).
