...
see [InfluxDB configuration options](/influxdb/v2.7/reference/config-options/).
    {{% /note %}}
...
2.  Use the [`influx auth create` command](/influxdb/v2.7/reference/cli/influx/auth/create/)
    to create an all access token.
    
    **Provide the following**:

    - `--all-access` flag
    - `--host` flag with your [InfluxDB host URL](/influxdb/v2.7/reference/urls/)
    - `-o, --org` or `--org-id` flags with your InfluxDB organization name or
      [ID](/influxdb/v2.7/organizations/view-orgs/#view-your-organization-id)
...
- **Request body**: JSON body with the following properties:
  ...
  - **orgID**: [InfluxDB organization ID](/influxdb/v2.7/organizations/view-orgs/#view-your-organization-id)
      ...
      - **resource**: JSON object that represents the InfluxDB resource to grant
        permission to. Each resource contains at least the following properties:
          ...
          - **type**: Resource type.
            ...
            [`/api/v2/resources` endpoint](/influxdb/v2.7/api/#operation/GetResources)._
...
credentials. Use the [`influx config create` command](/influxdb/v2.7/reference/cli/influx/config/create/)
to create a new CLI connection configuration. Include the following flags:
  ...
  - `-u, --host-url`: [InfluxDB host URL](/influxdb/v2.7/reference/urls/).
...
[Install and use the `influx` CLI](/influxdb/v2.7/tools/influx-cli/#set-up-the-influx-cli)._
...
- `INFLUX_HOST`: [InfluxDB host URL](/influxdb/v2.7/reference/urls/).
- `INFLUX_ORG`: InfluxDB organization name.
- `INFLUX_ORG_ID`: InfluxDB [organization ID](/influxdb/v2.7/organizations/view-orgs/#view-your-organization-id).
...
- `--host`: [InfluxDB host URL](/influxdb/v2.7/reference/urls/).
...
[active `influx` CLI configuration](/influxdb/v2.7/reference/cli/influx/#provide-required-authentication-credentials)
...
4.  Provide a bucket name (get-started) and select {{% caps %}}Never{{% /caps %}}
    to create a bucket with an infinite [retention period](/influxdb/v2.7/reference/glossary/#retention-period).
...
1.  If you haven't already, [download, install, and configure the `influx` CLI](/influxdb/v2.7/tools/influx-cli/).
2.  Use the [`influx bucket create` command](/influxdb/v2.7/reference/cli/influx/bucket/create/)
...
{{< page-nav prev="/influxdb/v2.7/get-started/" next="/influxdb/v2.7/get-started/write/" keepTab=true >}}
