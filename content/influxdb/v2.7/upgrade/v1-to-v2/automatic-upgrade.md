...
weight: 10
aliases:
  - /influxdb/v2.7/reference/upgrading/influxd-upgrade-guide/
related:
  - /influxdb/v2.7/reference/cli/influxd/upgrade/
  - /influxdb/v2.7/upgrade/v1-to-v2/manual-upgrade/
  - /influxdb/v2.7/upgrade/v1-to-v2/docker/
---
...
3. Writes existing data and write ahead log (WAL) files into InfluxDB {{< current-version >}} [buckets](/influxdb/v2.7/reference/glossary/#bucket).
4. Creates [database and retention policy (DBRP) mappings](/influxdb/v2.7/reference/api/influxdb-1x/dbrp/) required to query data with InfluxQL.
...
[Migrate continuous queries to tasks](/influxdb/v2.7/upgrade/v1-to-v2/migrate-cqs/).
...
- Kapacitor Batch-style TICKscripts work with the 1.x read compatible API.
  Existing Kapacitor user credentials should continue to work using the [1.x compatibility API](/influxdb/v2.7/reference/api/influxdb-1x/).
...
You can continue to use your existing dashboards and visualization tools with InfluxDB {{< current-version >}} via the [1.x read compatibility API](/influxdb/v2.7/reference/api/influxdb-1x/).
The upgrade process creates [DBRP mappings](/influxdb/v2.7/reference/api/influxdb-1x/dbrp/) to ensure existing users can execute InfluxQL queries with the appropriate permissions.
...
[create an InfluxDB {{< current-version >}} scraper](/influxdb/v2.7/write-data/no-code/scrape-data/manage-scrapers/create-a-scraper/)
...
1. Download and install in the [`influx` CLI](/influxdb/v2.7/tools/influx-cli/).
2. Set up your [`influx` CLI authentication credentials](/influxdb/v2.7/tools/influx-cli/#set-up-the-influx-cli).
3. Run the `influx v1 shell` command.

For more information see:

- [Use the InfluxQL shell](/influxdb/v2.7/tools/influxql-shell/)
...
[`influx v1 auth list`](/influxdb/v2.7/reference/cli/influx/v1/auth/list/).
...
1. [Manually create a 1.x-compatible authorization](/influxdb/v2.7/upgrade/v1-to-v2/manual-upgrade/#create-a-1x-compatible-authorization).
