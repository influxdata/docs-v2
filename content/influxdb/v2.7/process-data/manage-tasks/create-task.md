...
weight: 201
related:
  - /influxdb/v2.7/reference/cli/influx/task/create
---

Create tasks with the InfluxDB user interface (UI), `influx` command line interface (CLI), or `/api/v2` API.

_Before creating a task, review the [basics for writing a task](/influxdb/v2.7/process-data/get-started)._
...
4. Specify the task options. See [Task options](/influxdb/v2.7/process-data/task-options)
...
3. In the left panel, specify the task options.
   See [Task options](/influxdb/v2.7/process-data/task-options) for detailed information about each option.
...
When you save the task, the Task UI uses the [task options](/influxdb/v2.7/process-data/task-options/) you specify in the **Task options** form to populate `option task = {task_options}` for you.
...
Use the [`/api/v2/tasks` InfluxDB API endpoint](/influxdb/v2.7/api/#operation/PostTasks) to create a task.

{{< api-endpoint method="POST" endpoint="http://localhost:8086/api/v2/tasks/" api-ref="/influxdb/v2.7/api/#operation/PostTasks" >}}
...
- **orgID**: your [InfluxDB organization ID](/influxdb/v2.7/organizations/view-orgs/#view-your-organization-id)
