...
combined and replaced by InfluxDB [buckets](/influxdb/v2.7/reference/glossary/#bucket).
...
- Writing to the [`/write` v1 compatibility endpoint](/influxdb/v2.7/reference/api/influxdb-1x/write/)
- {{% cloud-only %}}[Upgrading from InfluxDB 1.x to InfluxDB Cloud](/influxdb/v2.7/upgrade/v1-to-cloud/){{% /cloud-only %}}
- {{% oss-only %}}[Upgrading from InfluxDB 1.x to {{< current-version noSpan=true >}}](/influxdb/v2.7/upgrade/v1-to-v2/){{% /oss-only %}}
...
[Database and retention policy mapping – When creating a bucket](/influxdb/v2.7/reference/api/influxdb-1x/dbrp/#when-creating-a-bucket).

{{% /oss-only %}}

## Create DBRP mappings

Use the [`influx` CLI](/influxdb/v2.7/reference/cli/influx/) or the
[InfluxDB API](/influxdb/v2.7/reference/api/) to create DBRP mappings.
...
- {{< req "\*" >}} **org** and **token** to authenticate. We recommend setting your organization and token to your active InfluxDB connection configuration in the influx CLI, so you don't have to add these parameters to each command. To set up your active InfluxDB configuration, see [`influx config set`](/influxdb/v2.7/reference/cli/influx/config/set/).
- {{< req "\*" >}} **database name** to map
- {{< req "\*" >}} **retention policy** name to map
- {{< req "\*" >}} [Bucket ID](/influxdb/v2.7/organizations/buckets/view-buckets/#view-buckets-in-the-influxdb-ui) to map to
...
Use the [`/api/v2/dbrps` API endpoint](/influxdb/v2.7/api/#operation/PostDBRP) to create a new DBRP mapping.

{{< api-endpoint endpoint="http://localhost:8086/api/v2/dbrps" method="POST" api-ref="/influxdb/v2.7/api/#operation/PostDBRP" >}}

Include the following:

- **Request method:** `POST`
- **Headers:**
  - **Authorization:** `Token` schema with your InfluxDB [API token](/influxdb/v2.7/security/tokens/)
  - **Content-type:** `application/json`
- **Request body:** JSON object with the following fields:  
  {{< req type="key" >}}
  - {{< req "\*" >}} **bucketID:** [bucket ID](/influxdb/v2.7/organizations/buckets/view-buckets/)
  - {{< req "\*" >}} **database:** database name
  - **default:** set the provided retention policy as the default retention policy for the database
  - {{< req "\*" >}} **org** or **orgID:** organization name or [organization ID](/influxdb/v2.7/organizations/view-orgs/#view-your-organization-id)
...
Use the [`influx` CLI](/influxdb/v2.7/reference/cli/influx/) or the [InfluxDB API](/influxdb/v2.7/reference/api/)
...
Use the [`influx v1 dbrp list` command](/influxdb/v2.7/reference/cli/influx/v1/dbrp/list/) to list DBRP mappings.

{{% note %}}
The examples below assume that your organization and API token are
provided by the active [InfluxDB connection configuration](/influxdb/v2.7/reference/cli/influx/config/) in the `influx` CLI.
...
{{% /tab-content %}}
{{% tab-content %}}
Use the [`/api/v2/dbrps` API endpoint](/influxdb/v2.7/api/#operation/GetDBRPs) to list DBRP mappings.

{{< api-endpoint endpoint="http://localhost:8086/api/v2/dbrps" method="GET" api-ref="/influxdb/v2.7/api/#operation/GetDBRPs" >}}
...
- **Query parameters:**  
  {{< req type="key" >}}
  - {{< req "\*" >}} **orgID:** [organization ID](/influxdb/v2.7/organizations/view-orgs/#view-your-organization-id)
  - **bucketID:** [bucket ID](/influxdb/v2.7/organizations/buckets/view-buckets/) _(to list DBRP mappings for a specific bucket)_
...
Use the [`influx` CLI](/influxdb/v2.7/reference/cli/influx/) or the
[InfluxDB API](/influxdb/v2.7/reference/api/) to update a DBRP mapping.
...
Use the [`influx v1 dbrp update` command](/influxdb/v2.7/reference/cli/influx/v1/dbrp/update/)
...
Use the [`/api/v2/dbrps/{dbrpID}` API endpoint](/influxdb/v2.7/api/#operation/GetDBRPs) to update DBRP mappings.

{{< api-endpoint endpoint="http://localhost:8086/api/v2/dbrps/{dbrpID}" method="PATCH" api-ref="/influxdb/v2.7/api/#operation/PatchDBRPID" >}}
...
- **Headers:**
  - {{< req "\*" >}} **Authorization:** `Token` schema with your InfluxDB [API token](/influxdb/v2.7/security/tokens/)
- **Path parameters:**
...
Use the [`influx` CLI](/influxdb/v2.7/reference/cli/influx/) or the
[InfluxDB API](/influxdb/v2.7/reference/api/) to delete a DBRP mapping.
...
Use the [`influx v1 dbrp delete` command](/influxdb/v2.7/reference/cli/influx/v1/dbrp/delete/)
...
Use the [`/api/v2/dbrps/{dbrpID}` API endpoint](/influxdb/v2.7/api/#operation/DeleteDBRPID) to delete a DBRP mapping.

{{< api-endpoint endpoint="http://localhost:8086/api/v2/dbrps/{dbrpID}" method="DELETE" api-ref="/influxdb/v2.7/api/#operation/DeleteDBRPID" >}}
