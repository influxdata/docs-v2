...
{{% note %}}
Command examples use the [NOAA water sample data](/influxdb/v2.7/reference/sample-data/#noaa-water-sample-data).
{{% /note %}}

## SHOW SERIES

Return a list of [series](/influxdb/v2.7/reference/glossary/#series) for
the specified [database](/influxdb/v2.7/reference/glossary/#database).
...
- `ON <database_name>` is optional.
  ...
  [InfluxDB API](/influxdb/v2.7/reference/api/influxdb-1x/) request.
...
See [Explore data using InfluxQL](/influxdb/v2.7/query-data/influxql/explore-data/) for documentation on the
[`FROM` clause](/influxdb/v2.7/query-data/influxql/explore-data/select/#from-clause),
[`LIMIT` clause](/influxdb/v2.7/query-data/influxql/explore-data/limit-and-slimit/),
[`OFFSET` clause](/influxdb/v2.7/query-data/influxql/explore-data/offset-and-soffset/),
and [Regular Expressions](/influxdb/v2.7/query-data/influxql/explore-data/regular-expressions/).
...
The query's output is similar to the [line protocol](/influxdb/v2.7/reference/syntax/line-protocol/) format.
Everything before the first comma is the [measurement](/influxdb/v2.7/reference/glossary/#measurement) name.
Everything after the first comma is either a [tag key](/influxdb/v2.7/reference/glossary/#tag-key) or a [tag value](/influxdb/v2.7/reference/glossary/#tag-value).
...
Returns a list of [measurements](/influxdb/v2.7/reference/glossary/#measurement)
for the specified [database](/influxdb/v2.7/reference/glossary/#database).
...
Returns a list of [tag keys](/influxdb/v2.7/reference/glossary/#tag-key)
associated with the specified [database](/influxdb/v2.7/reference/glossary/#database).
...
- `ON <database_name>` is optional.
  If the query does not include `ON <database_name>`, you must specify the
  database with `db` query string parameter in the [InfluxDB API](/influxdb/v2.7/reference/api/influxdb-1x/) request.
...
Returns the list of [tag values](/influxdb/v2.7/reference/glossary/#tag-value)
for the specified [tag key(s)](/influxdb/v2.7/reference/glossary/#tag-key) in the database.
...
- `ON <database_name>` is optional.
  If the query does not include `ON <database_name>`, you must specify the
  database with the `db` query string parameter in the [InfluxDB API](/influxdb/v2.7/reference/api/influxdb-1x/) request.
...
Returns the [field keys](/influxdb/v2.7/reference/glossary/#field-key) and the
[data type](/influxdb/v2.7/reference/glossary/#data-type) of their
[field values](/influxdb/v2.7/reference/glossary/#field-value).
...
- `ON <database_name>` is optional.
  If the query does not include `ON <database_name>`, you must specify the
  database with `USE <database_name>` when using the [InfluxQL shell](/influxdb/v2.7/tools/influxql-shell/)
  or with the `db` query string parameter in the
  [InfluxDB 1.x compatibility API](/influxdb/v2.7/reference/api/influxdb-1x/) request.
- The `FROM` clause is optional.
  See the Data Exploration page for documentation on the
[`  FROM` clause](/influxdb/v2.7/query-data/influxql/explore-data/select/#from-clause).

{{% note %}}
**Note:** A field's data type [can differ](/influxdb/v2.7/reference/faq/#how-does-influxdb-handle-field-type-discrepancies-across-shards) across
[shards](/influxdb/v2.7/reference/glossary/#shard).
...
Field value [data types](/influxdb/v2.7/reference/glossary/#data-type)
cannot differ within a [shard](/influxdb/v2.7/reference/glossary/#shard) but they
...
When you filter meta queries by time, you may see results outside of your specified time. Meta query results are filtered at the shard level, so results can be approximately as granular as your shard group duration. If your time filter spans multiple shards, you'll get results from all shards with points in the specified time range. To review your shards and timestamps on points in the shard, run `SHOW SHARDS`. To learn more about shards and their duration, see [recommended shard groups durations](/influxdb/v2.7/reference/internals/shards/#shard-group-duration).
