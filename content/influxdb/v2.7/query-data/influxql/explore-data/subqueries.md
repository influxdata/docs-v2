...
The main query surrounds the subquery and requires at least the [`SELECT` clause](/influxdb//v2.4/query-data/influxql/explore-data/select/) and the [`FROM` clause](/influxdb/v2.7/query-data/influxql/explore-data/select/#from-clause).
...
Notice that the main query specifies `difference` as the field key in the [`MEAN()`](/influxdb/v2.7/query-data/influxql/functions/aggregates/#mean) function.
...
Notice that the main query specifies `water_level_derivative`, not `water_level` or `derivative`, as the field key in the [`SUM()`](/influxdb/v2.7/query-data/influxql/functions/aggregates/#sum) function.
...
InfluxQL does not support multiple [`SELECT` statements](/influxdb/v2.7/query-data/influxql/explore-data/select/) per subquery:
