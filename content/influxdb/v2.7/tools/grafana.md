...
aliases:
  - /influxdb/v2.7/visualize-data/other-tools/grafana/
related:
...
1. {{% oss-only %}}[Start InfluxDB](/influxdb/v2.7/install/#start-and-configure-influxdb).{{% /oss-only %}}
...
1.  Under **HTTP**, enter the following:
    ...
    - **URL**: Your
      {{% oss-only %}}[InfluxDB URL](/influxdb/v2.7/reference/urls/).{{% /oss-only %}}
      {{% cloud-only %}}[InfluxDB Cloud region URL](/influxdb/v2.7/reference/regions/).{{% /cloud-only %}}
...
2.  Under **InfluxDB Details**, enter the following:
    ...
    - **Organization**: Your InfluxDB [organization name **or** ID](/influxdb/v2.7/organizations/view-orgs/).
    - **Token**: Your InfluxDB [API token](/influxdb/v2.7/security/tokens/).
    - **Default Bucket**: The default [bucket](/influxdb/v2.7/organizations/buckets/) to use in Flux queries.
...
1. [Authenticate with InfluxDB {{< current-version >}} tokens](/influxdb/v2.7/security/tokens/).
...
InfluxDB {{< current-version >}} (following an [official upgrade guide](/influxdb/v2.7/upgrade/v1-to-v2/)):
...
1. If your InfluxDB 1.x instance required authentication,
   ...
   Otherwise, use [InfluxDB v2 token authentication](/influxdb/v2.7/security/tokens/).
2. [Manually create DBRP mappings](#view-and-create-influxdb-dbrp-mappings).
...
Use the [`influx v1 auth list`](/influxdb/v2.7/reference/cli/influx/v1/auth/list/)
...
Use the [`influx v1 auth create` command](/influxdb/v2.7/reference/cli/influx/v1/auth/create/)
to grant read/write permissions to specific buckets. Provide the following:

- [bucket IDs](/influxdb/v2.7/organizations/buckets/view-buckets/) to grant read
...
InfluxDB {{< current-version >}} [buckets](/influxdb/v2.7/reference/glossary/#bucket).
...
Use the [`influx v1 dbrp list`](/influxdb/v2.7/reference/cli/influx/v1/dbrp/list/)
...
_For more information about DBRP mapping, see [Database and retention policy mapping](/influxdb/v2.7/reference/api/influxdb-1x/dbrp/)._
...
1. Under **HTTP**, enter the following:
    ...
    - **URL**: Your [InfluxDB URL](/influxdb/v2.7/reference/urls/).
...
2. Configure InfluxDB authentication:
    ...
    - ##### Token authentication
        ...
        - Under **Custom HTTP Headers**, select **{{< icon "plus" >}}Add Header**. Provide your InfluxDB API token:
          ...
          - **Value**: Use the `Token` schema and provide your [InfluxDB API token](/influxdb/v2.7/security/tokens/).
            For example:
