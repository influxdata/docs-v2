...
products: [oss]
related:
  - /influxdb/v2.4/reference/cli/influxd/recovery/
---
...
{{% note %}}
If you used a [custom `bolt-path`](/influxdb/v2.4/reference/config-options/#bolt-path) when starting InfluxDB, provide your custom bolt path to the `influx recovery user list` command with the `--bolt-path` flag.
{{% /note %}}
