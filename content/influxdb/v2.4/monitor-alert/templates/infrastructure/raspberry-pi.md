...
- one [bucket](/influxdb/v2.4/reference/glossary/#bucket): `rasp-pi` (7d retention)
...
- one [Telegraf configuration](/influxdb/v2.4/telegraf-configs/)
- one [dashboard](/influxdb/v2.4/reference/glossary/#dashboard): Raspberry Pi System
...
2.  [Install Telegraf](/{{< latest "telegraf" >}}/introduction/installation/) on
    ...
    [InfluxDB {{% cloud-only %}}Cloud{{% /cloud-only %}} API](/influxdb/v2.4/reference/api/).
3. Add the following environment variables to your Telegraf environment:

    - `INFLUX_HOST`: {{% oss-only %}}Your [InfluxDB URL](/influxdb/v2.4/reference/urls/){{% /oss-only %}}
      {{% cloud-only %}}Your [InfluxDB Cloud region URL](/influxdb/cloud/reference/regions/){{% /cloud-only %}}
    - `INFLUX_TOKEN`: Your [InfluxDB {{% cloud-only %}}Cloud{{% /cloud-only %}} API token](/influxdb/v2.4/security/tokens/)
