...
related:
  - /influxdb/v2.4/write-data/no-code/use-telegraf/manual-config/
  - /influxdb/v2.4/write-data/no-code/use-telegraf/auto-config/
...
For more information, see [Automatically configure Telegraf](/influxdb/v2.4/write-data/no-code/use-telegraf/auto-config/).

## Use the influx CLI
Use the [`influx telegrafs create` command](/influxdb/v2.4/reference/cli/influx/telegrafs/create/)
...
1. Create a custom Telegraf configuration file that includes the `outputs.influxdb_v2`
   output plugin. _See [Manually configure Telegraf](/influxdb/v2.4/write-data/no-code/use-telegraf/manual-config/)_
