...
menu:
  influxdb_2_4:
    ...
    parent: Write data
influxdb/v2.4/tags: [write, line protocol, errors]
related:
  - /influxdb/v2.4/api/#tag/Write, InfluxDB API /write endpoint
  - /influxdb/v2.4/reference/internals
...
- `400` **Bad request**: The [line protocol](/influxdb/v2.4/reference/syntax/line-protocol/) data in the request was malformed.
        The response body contains the first malformed line in the data. All request data was rejected and not written.
- `401` **Unauthorized**: May indicate one of the following:
  - [`Authorization: Token` header](/influxdb/v2.4/api-guide/api_intro/#authentication) is missing or malformed.
  - [API token](/influxdb/v2.4/api-guide/api_intro/#authentication) value is missing from the header.
  - API token does not have sufficient permissions to write to the organization and the bucket. For more information about token types and permissions, see [Manage API tokens](/influxdb/v2.4/security/tokens/).
...
- Verify all lines contain valid syntax ([line protocol](/influxdb/v2.4/reference/syntax/line-protocol/) or [CSV](/influxdb/v2.4/reference/syntax/annotated-csv/)).
- Verify the timestamps match the [precision parameter](/influxdb/v2.4/write-data/#timestamp-precision).
- Minimize payload size and network errors by [optimizing writes](/influxdb/v2.4/write-data/best-practices/optimize-writes/).
...
Check for [field type](/influxdb/v2.4/reference/key-concepts/data-elements/#field-value) differences between the missing data point and other points that have the same [series](/influxdb/v2.4/reference/key-concepts/data-elements/#series)--for example, did you attempt to write `string` data to an `int` field?
