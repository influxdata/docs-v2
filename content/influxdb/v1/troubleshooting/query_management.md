...
> The commands and configurations provided on this page are for **Influx Query Language (InfluxQL) only** -- **no equivalent set of Flux commands and configurations currently exists**. For the most current Flux documentation, see [Get started with Flux](/influxdb/v1/flux/get-started/).
...
Where `qid` is the query ID, displayed in the [`SHOW QUERIES`](/influxdb/v1/troubleshooting/query_management/#list-currently-running-queries-with-show-queries) output:
...
[coordinator](/influxdb/v1/administration/config/#query-management-settings) section of the
configuration file.
...
This setting is a [duration literal](/influxdb/v1/query_language/spec/#durations).
...
Use this value with [`KILL QUERY`](/influxdb/v1/troubleshooting/query_management/#stop-currently-running-queries-with-kill-query).
