...
- To **write to a database using the InfluxDB 2.0 API (compatible with InfluxDB 1.8+)**, send `POST` requests to the [`/api/v2/write` endpoint](/influxdb/v1/api/#api-v2-write-http-endpoint):
...
See the [InfluxDB API Reference](/influxdb/v1/api/#write-http-endpoint) documentation for a complete list of the available query parameters.
...
- **Fields (required)**: Field keys are required and are always strings, and, [by default](/influxdb/v1/write_protocols/line_protocol_reference/#data-types), field values are floats.
...
The data in the file should follow the [InfluxDB line protocol syntax](/influxdb/v1/write_protocols/write_syntax/).
...
This simple, easy to understand system uses HTTP as a transfer method for [InfluxQL](/influxdb/v1/query_language/spec/).
...
For more information about writing data with the InfluxDB API, please see the [InfluxDB API reference](/influxdb/v1/api/#write-http-endpoint).
