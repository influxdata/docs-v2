...
- Add [`config`](/influxdb/v1/api/#running-configuration) and [`cq` (continuous query) statistics](/influxdb/v1/api/#continuous-query-statistics) to the `/debug/vars` endpoint.
...
- Add [`time_format` parameter](/influxdb/v1/api/#query-data-with-a-select-statement-and-the-time_format-parameter) for the HTTP API.
...
- Strip double quotes from measurement names in the [`/api/v2/delete`
  compatibility API](/influxdb/v1/api/#apiv2delete-http-endpoint) before
...
- **InfluxDB v2 compatibility API updates:**
  - Partially support create, retrieve, update, delete, and list operations in the
    [`/v2/api/buckets` compatibility API](/influxdb/v1/api/#influxdb-2x-api-compatibility-endpoints)
    and correctly handle requests to unsupported `/v2/api/buckets` endpoints.
  - Implement the [`v2/api/delete` compatibility API](/influxdb/v1/api/#apiv2delete-http-endpoint).
...
- Add the ability to [track number of values](/platform/monitoring/influxdata-platform/tools/measurements-internal/#valueswrittenok) written via the [`/debug/vars` HTTP endpoint](/influxdb/v1/api/#debugvars-http-endpoint).
...
- [InfluxDB 2.x API compatibility endpoints](/influxdb/v1/api/#influxdb-2x-api-compatibility-endpoints) are now part of the InfluxDB 1.x line.  
