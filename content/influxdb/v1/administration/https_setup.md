...
v2: /influxdb/v2/security/enable-tls/
---
...
InfluxData [strongly recommends](/influxdb/v1/administration/security/) enabling HTTPS, especially if you plan on sending requests to InfluxDB over a network.
...
For more information, see [Transport Layer Security (TLS) configuration settings](/influxdb/v1/administration/config#transport-layer-security-tls-settings).
...
Verify that HTTPS is working by connecting to InfluxDB with the [CLI tool](/influxdb/v1/tools/shell/):
...
By default, InfluxDB supports the values for TLS `ciphers`, `min-version`, and `max-version` listed in the [Constants section of the Go `crypto/tls` package documentation](https://golang.org/pkg/crypto/tls/#pkg-constants) and depends on the version of Go used to build InfluxDB. You can configure InfluxDB to support a restricted list of TLS cipher suite IDs and versions. For more information, see [Transport Layer Security (TLS) settings `[tls]`](/influxdb/v1/administration/config#transport-layer-security-tls-settings).
...
> Instructions are provided in the [InfluxDB Enterprise HTTPS Setup guide](/enterprise_influxdb/v1/guides/https_setup/#set-up-https-with-a-self-signed-certificate).
...
Connecting [Telegraf](/telegraf/v1/) to an InfluxDB instance that's using
