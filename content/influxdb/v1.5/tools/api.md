...
| Verb  | Query Type |
| :---- | :--------- |
| GET   | Use for all queries that start with: <br><br> [`SELECT`](/influxdb/v1.5/query_language/spec/#select)* <br><br> [`SHOW`](/influxdb/v1.5/query_language/spec/#show-continuous-queries)   |
| POST  | Use for all queries that start with: <br><br> [`ALTER`](/influxdb/v1.5/query_language/spec/#alter-retention-policy) <br><br> [`CREATE`](/influxdb/v1.5/query_language/spec/#create-continuous-query) <br><br> [`DELETE`](/influxdb/v1.5/query_language/spec/#delete) <br><br> [`DROP`](/influxdb/v1.5/query_language/spec/#drop-continuous-query) <br><br> [`GRANT`](/influxdb/v1.5/query_language/spec/#grant) <br><br> [`KILL`](/influxdb/v1.5/query_language/spec/#kill-query) <br><br> [`REVOKE`](/influxdb/v1.5/query_language/spec/#revoke) |

\* The only exceptions are `SELECT` queries that include an [`INTO` clause](/influxdb/v1.5/query_language/data_exploration/#the-into-clause).
...
The `mymeas` [measurement](/influxdb/v1.5/concepts/glossary/#measurement) has two points.
The first point has the [timestamp](/influxdb/v1.5/concepts/glossary/#timestamp) `2017-03-01T00:16:18Z`, a `myfield` value of `33.1`, and no tag values for the `mytag1` and `mytag2` [tag keys](/influxdb/v1.5/concepts/glossary/#tag-key).
...
The same query in InfluxDB's [Command Line Interface](/influxdb/v1.5/tools/shell/) (CLI) returns the following table:
```
name: mymeas
...
`SELECT` queries that include and [`INTO` clause](/influxdb/v1.5/query_language/data_exploration/#the-into-clause) require a `POST` request.

The response shows that InfluxDB writes two points to the `newmeas` [measurement](/influxdb/v1.5/concepts/glossary/#measurement).
Note that the system uses epoch 0 (`1970-01-01T00:00:00Z`) as a [null timestamp equivalent](/influxdb/v1.5/troubleshooting/frequently-asked-questions/#why-does-my-query-return-epoch-0-as-the-timestamp).
...
A successful [`CREATE DATABASE` query](/influxdb/v1.5/query_language/database_management/#create-database) returns no additional information.
...
| db=\<database_name> | Required for database-dependent queries (most [`SELECT`](/influxdb/v1.5/query_language/spec/#select) queries and [`SHOW`](/influxdb/v1.5/query_language/spec/#show-continuous-queries) queries require this parameter). | Sets the target [database](/influxdb/v1.5/concepts/glossary/#database) for the query. |
...
| q=\<query> | Required | InfluxQL string to execute.  See also [Request Body](/influxdb/v1.5/tools/api/#request-body). |
| u=\<username> | Optional if you haven't [enabled authentication](/influxdb/v1.5/query_language/authentication_and_authorization/#set-up-authentication). Required if you've enabled authentication.* | Sets the username for authentication if you've enabled authentication. The user must have read access to the database. Use with the query string parameter `p`. |

\* InfluxDB does not truncate the number of rows returned for requests without the `chunked` parameter.
That behavior is configurable; see the [`max-row-limit`](/influxdb/v1.5/administration/config/#max-row-limit-0) configuration option for more information.

\** The HTTP API also supports basic authentication.
Use basic authentication if you've [enabled authentication](/influxdb/v1.5/query_language/authentication_and_authorization/#set-up-authentication)
...
[InfluxQL](/influxdb/v1.5/query_language/) syntax.
...
the [`WHERE` clause](/influxdb/v1.5/query_language/data_exploration/#the-where-clause).
...
InfluxDB stores [tag values](/influxdb/v1.5/concepts/glossary/#tag-value) as strings they and must be double quoted in the request.
...
The value `30` does not require double quotes because `myfield` stores numerical [field values](/influxdb/v1.5/concepts/glossary/#field-value).
...
| rp=\<retention_policy_name> | Optional | Sets the target [retention policy](/influxdb/v1.5/concepts/glossary/#retention-policy-rp) for the write. InfluxDB writes to the `DEFAULT` retention policy if you do not specify a retention policy. |
...
[Line Protocol](/influxdb/v1.5/concepts/glossary/#line-protocol) format.
