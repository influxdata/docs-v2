...
All of these data is in the `NOAA_water_database` [database](/influxdb/v1.5/concepts/glossary/#database).
...
> **Note:** The quoting syntax for queries differs from the [line protocol](/influxdb/v1.5/concepts/glossary/#line-protocol).
...
Single quote [tag values](/influxdb/v1.5/concepts/glossary/#tag-value) in
the `WHERE` clause.
...
The query returns the three oldest [points](/influxdb/v1.5/concepts/glossary/#point) (determined by timestamp) from one
...
Include a [duration literal](/influxdb/v1.5/query_language/spec/#durations)
...
Add (`+`) or subtract (`-`) a time from a timestamp with a [duration literal](/influxdb/v1.5/query_language/spec/#durations).
...
The query returns data with timestamps that occur between August 18, 2015
at 00:00:00 and August 18, 2015 at 00:12:00.
The `s` [duration literal](/influxdb/v1.5/query_language/spec/#durations) at the
...
## Relative time
Use [`now()`](/influxdb/v1.5/concepts/glossary/#now) to query data with [timestamps](/influxdb/v1.5/concepts/glossary/#timestamp) relative to the server's current timestamp.
...
The whitespace between `-` or `+` and the [duration literal](/influxdb/v1.5/query_language/spec/#durations) is required.
...
See the [Frequently Asked Questions](/influxdb/v1.5/troubleshooting/frequently-asked-questions/#why-is-my-query-with-a-where-or-time-clause-returning-empty-results)
document for more information.

### Issue 2: Querying data that occur after `now()` with a `GROUP BY time()` clause

Most `SELECT` statements have a default time range between [`1677-09-21 00:12:43.145224194` and `2262-04-11T23:47:16.854775806Z` UTC](/influxdb/v1.5/troubleshooting/frequently-asked-questions/#what-are-the-minimum-and-maximum-timestamps-that-influxdb-can-store).
For `SELECT` statements with a [`GROUP BY time()` clause](#group-by-time-intervals), the default time
range is between `1677-09-21 00:12:43.145224194` UTC and [`now()`](/influxdb/v1.5/concepts/glossary/#now).
...
Use the [CLI](/influxdb/v1.5/tools/shell/) to write a point to the `NOAA_water_database` that occurs after `now()`:
```
...
Specify alternative formats with the
[`epoch` query string parameter](/influxdb/v1.5/tools/api/#query-string-parameters).
...
* [field keys](/influxdb/v1.5/concepts/glossary/#field-key) and [tag keys](/influxdb/v1.5/concepts/glossary/#tag-key) in the [`SELECT` clause](#the-basic-select-statement)
* [measurements](/influxdb/v1.5/concepts/glossary/#measurement) in the [`FROM` clause](#the-basic-select-statement)
* [tag values](/influxdb/v1.5/concepts/glossary/#tag-value) and string [field values](/influxdb/v1.5/concepts/glossary/#field-value) in the [`WHERE` clause](#the-where-clause).
* [tag keys](/influxdb/v1.5/concepts/glossary/#tag-key) in the [`GROUP BY` clause](#group-by-tags)
...
The query selects all [field keys](/influxdb/v1.5/concepts/glossary/#field-key)
and [tag keys](/influxdb/v1.5/concepts/glossary/#tag-key) that include an `l`.
...
The query uses an InfluxQL [function](/influxdb/v1.5/query_language/functions/)
to return the distinct [field values](/influxdb/v1.5/concepts/glossary/#field-value)
...
The query uses an InfluxQL [function](/influxdb/v1.5/query_language/functions/)
to calculate the average `degrees` for every [measurement](/influxdb/v1.5/concepts/glossary#measurement) in the `NOAA_water_database`
[database](/influxdb/v1.5/concepts/glossary#database) that contains the word `temperature`.
...
The query uses an InfluxQL [function](/influxdb/v1.5/query_language/functions/)
to calculate the average `water_level` where the [tag value](/influxdb/v1.5/concepts/glossary#tag-value) of `location`
...
[tag](/influxdb/v1.5/concepts/glossary#tag) has no value.
Every data [point](/influxdb/v1.5/concepts/glossary#point) in the `NOAA_water_database` has a tag value for `location`.

It's possible to perform this same query without a regular expression.
See the
[Frequently Asked Questions](/influxdb/v1.5/troubleshooting/frequently-asked-questions/#how-do-i-select-data-with-a-tag-that-has-no-value)
document for more information.
...
The [`SELECT` clause](#the-basic-select-statement) supports specifying a [field's](/influxdb/v1.5/concepts/glossary/#field) type and basic cast
...
[Field values](/influxdb/v1.5/concepts/glossary/#field-value) can be floats, integers, strings, or booleans.
...
In most cases, InfluxDB rejects any writes that attempt to write a [field value](/influxdb/v1.5/concepts/glossary/#field-value)
to a field that previously accepted field values of a different type.
>
It is possible for field value types to differ across [shard groups](/influxdb/v1.5/concepts/glossary/#shard-group).
...
Currently, InfluxDB supports casting [field values](/influxdb/v1.5/concepts/glossary/#field-value) from integers to
...
The query returns the integer form of `water_level`'s float [field values](/influxdb/v1.5/concepts/glossary/#field-value).
...
<br>
# Merge Behavior
In InfluxDB, queries merge [series](/influxdb/v1.5/concepts/glossary/#series)
automatically.

### Example

The `h2o_feet` [measurement](/influxdb/v1.5/concepts/glossary/#measurement) in the `NOAA_water_database` is part of two [series](/influxdb/v1.5/concepts/glossary/#series).
The first series is made up of the `h2o_feet` measurement and the `location = coyote_creek` [tag](/influxdb/v1.5/concepts/glossary/#tag).
The second series is made of up the `h2o_feet` measurement and the `location = santa_monica` tag.

The following query automatically merges those two series when it calculates the [average](/influxdb/v1.5/query_language/functions/#mean) `water_level`:
...
In InfluxDB's [CLI](/influxdb/v1.5/tools/shell/):
...
With InfluxDB's [HTTP API](/influxdb/v1.5/tools/api/):
...
#### Example 1: Calculate the [`SUM()`](/influxdb/v1.5/query_language/functions/#sum) of several [`MAX()`](/influxdb/v1.5/query_language/functions/#max) values
```
...
#### Example 2: Calculate the [`MEAN()`](/influxdb/v1.5/query_language/functions/#mean) difference between two fields
```
...
#### Example 3: Calculate several [`MEAN()`](/influxdb/v1.5/query_language/functions/#mean) values and place a condition on those mean values
```
...
#### Example 4: Calculate the [`SUM()`](/influxdb/v1.5/query_language/functions/#sum) of several [`DERIVATIVE()`](/influxdb/v1.5/query_language/functions/#derivative) values
```
