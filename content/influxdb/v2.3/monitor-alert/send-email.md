...
weight: 104
influxdb/v2.3/tags: [alert, email, notifications, check]
related:
...
1. [Create a check](/influxdb/v2.3/monitor-alert/checks/create/#create-a-check-in-the-influxdb-ui) to identify the data to monitor and the status to alert on.
...
3. [Create an alert email task](#create-an-alert-email-task) to call your email service and send an alert email.
  ...
  {{% note %}}
  In the procedure below, we use the **Task** page in the InfluxDB UI (user interface) to create a task. Explore other ways to [create a task](/influxdb/v2.3/process-data/manage-tasks/create-task/).
  {{% /note %}}
...
4.  In the right panel, enter the following detail in your **task script** (see [examples below](#examples)):
    ...
    - (Optional) Store your API key as a secret for reuse.
      First, [add your API key as a secret](/influxdb/v2.3/security/secrets/manage-secrets/add/),
