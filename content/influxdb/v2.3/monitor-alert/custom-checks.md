...
weight: 201
influxdb/v2.3/tags: [alerts, checks, tasks, Flux]
---

In the UI, you can create two kinds of [checks](/influxdb/v2.3/reference/glossary/#check):
[`threshold`](/influxdb/v2.3/monitor-alert/checks/create/#threshold-check) and
[`deadman`](/influxdb/v2.3/monitor-alert/checks/create/#deadman-check).
...
3. In the **Name** field, enter a descriptive name,
   ...
   For more detail, such as using cron syntax or including an offset, see [Task configuration options](/influxdb/v2.3/process-data/task-options/).
...
{{% note %}}
Use the [`/api/v2/checks/{checkID}/query` API endpoint](/influxdb/v2.3/api/#operation/DeleteDashboardsIDOwnersID)
...
[Create notification endpoints](/influxdb/v2.3/monitor-alert/notification-endpoints/create),
[Create notification rules](/influxdb/v2.3/monitor-alert/notification-rules/create),
and [Send alert email](/influxdb/v2.3/monitor-alert/send-email/)
{{% /note %}}
