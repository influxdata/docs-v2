...
description: >
  Use the InfluxDB UI or the [`influx` CLI](/influxdb/v2.3/reference/cli/influx/)
...
Use the InfluxDB user interface (UI) or the [`influx` CLI](/influxdb/v2.3/reference/cli/influx/)
...
For more information, see [Automatically configure Telegraf](/influxdb/v2.3/write-data/no-code/use-telegraf/auto-config/).

## Use the influx CLI
Use the [`influx telegrafs create` command](/influxdb/v2.3/reference/cli/influx/telegrafs/create/)
...
1. Create a custom Telegraf configuration file that includes the `outputs.influxdb_v2`
   output plugin. _See [Manually configure Telegraf](/influxdb/v2.3/write-data/no-code/use-telegraf/manual-config/)_
