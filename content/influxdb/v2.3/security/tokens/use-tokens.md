...
Authenticate requests using the [`influx` CLI](/influxdb/v2.2/reference/cli/influx/), API requests made with client libraries, and tools like `curl`.
...
{{% note %}}
See [here](/influxdb/v2.2/write-data/no-code/use-telegraf/auto-config/#configure-your-token-as-an-environment-variable)
...
Automatically manage and use tokens from the CLI using [`influx config`](/influxdb/v2.2/reference/cli/influx/config/).

### Use a token in an API request

Use tokens in [API requests](/influxdb/v2.2/api-guide/api_intro/#authentication).

### Use a token in Postman

Make authenticated requests with tokens [using Postman](/influxdb/v2.2/api-guide/postman/).
