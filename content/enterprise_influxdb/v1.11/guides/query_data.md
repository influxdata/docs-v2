...
menu:
  enterprise_influxdb_1_11:
    ...
    parent: Guides
aliases:
  - /enterprise_influxdb/v1.11/guides/querying_data/
v2: /influxdb/v2.0/query-data/
---


The InfluxDB API is the primary means for querying data in InfluxDB (see the [command line interface](/enterprise_influxdb/v1.11/tools/influx-cli/use-influx/) and [client libraries](/enterprise_influxdb/v1.11/tools/api_client_libraries/) for alternative ways to query the database).
...
The header row defines column labels for the table. The `cpu` [measurement](/enterprise_influxdb/v1.11/concepts/glossary/#measurement) has four points, each represented by one of the record rows. For example the first point has a [timestamp](/enterprise_influxdb/v1.11/concepts/glossary/#timestamp) of `2020-04-07T18:08:19`.  

### Flux

Check out the [Get started with Flux](/influxdb/v2.0/query-data/get-started/) to learn more about building queries with Flux.
For more information about querying data with the InfluxDB API using Flux, see the [API reference documentation](/enterprise_influxdb/v1.11/tools/api/#influxdb-2-0-api-compatibility-endpoints).
...
The example below uses the InfluxDB API to query the same database that you encountered in [Writing Data](/enterprise_influxdb/v1.11/guides/writing_data/).
...
Check out the [Data Exploration page](/enterprise_influxdb/v1.11/query_language/explore-data/) to get acquainted with InfluxQL.
For more information about querying data with the InfluxDB API using InfluxQL, see the [API reference documentation](/enterprise_influxdb/v1.11/tools/api/#influxdb-1-x-http-endpoints).
