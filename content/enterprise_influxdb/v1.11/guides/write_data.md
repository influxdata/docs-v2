...
menu:
  enterprise_influxdb_1_11:
    ...
    parent: Guides
aliases:
  - /enterprise_influxdb/v1.11/guides/writing_data/
v2: /influxdb/v2.0/write-data/
---

Write data into InfluxDB using the [command line interface](/enterprise_influxdb/v1.11/tools/influx-cli/use-influx/), [client libraries](/enterprise_influxdb/v1.11/clients/api/), and plugins for common data formats such as [Graphite](/enterprise_influxdb/v1.11/write_protocols/graphite/).
...
The data consists of the [measurement](/enterprise_influxdb/v1.11/concepts/glossary/#measurement) `cpu_load_short`, the [tag keys](/enterprise_influxdb/v1.11/concepts/glossary/#tag-key) `host` and `region` with the [tag values](/enterprise_influxdb/v1.11/concepts/glossary/#tag-value) `server01` and `us-west`, the [field key](/enterprise_influxdb/v1.11/concepts/glossary/#field-key) `value` with a [field value](/enterprise_influxdb/v1.11/concepts/glossary/#field-value) of `0.64`, and the [timestamp](/enterprise_influxdb/v1.11/concepts/glossary/#timestamp) `1434055562000000000`.
...
- To **write to a database using the InfluxDB 2.0 API (compatible with InfluxDB 1.8+)**, send `POST` requests to the [`/api/v2/write` endpoint](/enterprise_influxdb/v1.11/tools/api/#api-v2-write-http-endpoint):
...
See the [InfluxDB API Reference](/enterprise_influxdb/v1.11/tools/api/#write-http-endpoint) documentation for a complete list of the available query parameters.

The body of the POST or [InfluxDB line protocol](/enterprise_influxdb/v1.11/concepts/glossary/#influxdb-line-protocol) contains the time series data that you want to store. Data includes:
...
- **Fields (required)**: Field keys are required and are always strings, and, [by default](/enterprise_influxdb/v1.11/write_protocols/line_protocol_reference/#data-types), field values are floats.
...
The data in the file should follow the [InfluxDB line protocol syntax](/enterprise_influxdb/v1.11/write_protocols/write_syntax/).
...
This simple, easy to understand system uses HTTP as a transfer method for [InfluxQL](/enterprise_influxdb/v1.11/query_language/spec/).
...
Now that you know how to write data with the InfluxDB API, discover how to query them with the [Querying data](/enterprise_influxdb/v1.11/guides/querying_data/) guide!
For more information about writing data with the InfluxDB API, please see the [InfluxDB API reference](/enterprise_influxdb/v1.11/tools/api/#write-http-endpoint).
