...
You must have [admin permissions](/enterprise_influxdb/v1.11/administration/manage/users-and-permissions/permissions/#admin) to set up FGA.
...
1. [Enable authentication](/enterprise_influxdb/v1.11/administration/configure/security/authentication/) in your InfluxDB configuration file.
...
2. Create users through the InfluxDB query API.
    ...
    For more information, see [User management commands](/enterprise_influxdb/v1.11/administration/manage/users-and-permissions/authorization-influxql/#user-management-commands).
...
5. [Create roles](#manage-roles) to grant permissions to users assigned to a role.
    ...
    {{% note %}}
For an overview of how users and roles work in InfluxDB Enterprise, see [InfluxDB Enterprise users](/enterprise_influxdb/v1.11/features/users/).
    {{% /note %}}
...
Grants for specific series also apply to [meta queries](/enterprise_influxdb/v1.11/query_language/schema_exploration).
