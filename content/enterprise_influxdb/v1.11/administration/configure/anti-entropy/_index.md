...
aliases:
  - /enterprise_influxdb/v1.11/guides/Anti-Entropy/
  - /enterprise_influxdb/v1.11/administration/anti-entropy/
menu:
...
By default, the service performs consistency checks every 5 minutes. This interval can be modified in the [`anti-entropy.check-interval`](/enterprise_influxdb/v1.11/administration/config-data-nodes/#check-interval) configuration setting.
...
[replication factor](/enterprise_influxdb/v1.11/concepts/glossary/#replication-factor)
...
3. Entropy is reported by the [Anti-Entropy `/status` API](/enterprise_influxdb/v1.11/administration/anti-entropy-api/#get-status) or with the `influxd-ctl entropy show` command.
...
The configuration settings for the Anti-Entropy service are described in [Anti-Entropy settings](/enterprise_influxdb/v1.11/administration/configure/config-data-nodes/#anti-entropy-ae-settings) section of the data node configuration.
...
> [`influxd-ctl kill-copy-shard`](/enterprise_influxdb/v1.11/tools/influxd-ctl/#kill-copy-shard) command.

## InfluxDB Anti-Entropy API

The Anti-Entropy service uses an API for managing and monitoring entropy.
Details on the available API endpoints can be found in [The InfluxDB Anti-Entropy API](/enterprise_influxdb/v1.11/administration/anti-entropy-api).
...
_View the [Replacing Data Nodes](/enterprise_influxdb/v1.11/guides/replacing-nodes/#replace-data-nodes-in-an-influxdb-enterprise-cluster) documentation for instructions on replacing data nodes in your InfluxDB Enterprise cluster._
...
shard using the [`influxd-ctl truncate-shards` command](/enterprise_influxdb/v1.11/tools/influxd-ctl/#truncate-shards),
