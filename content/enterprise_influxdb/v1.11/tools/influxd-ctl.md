...
menu:
  enterprise_influxdb_1_11:
    ...
    parent: Tools
aliases:
    - /enterprise_influxdb/v1.11/tools/influxd/restore/
    - /enterprise_influxdb/v1.11/tools/influxd/backup/
---
...
The `influxd-ctl` utility is available on all [meta nodes](/enterprise_influxdb/v1.11/concepts/glossary/#meta-node).
...
If authentication is enabled in the cluster's [meta node configuration files](/enterprise_influxdb/v1.11/administration/configure/config-meta-nodes/#auth-enabled)
and [data node configuration files](/enterprise_influxdb/v1.11/administration/config-data-nodes/#meta-auth-enabled)
...
[meta node configuration files](/enterprise_influxdb/v1.11/administration/config-meta-nodes/#auth-enabled-false)
and [data node configuration files](/enterprise_influxdb/v1.11/administration/configure/config-data-nodes/#meta-auth-enabled)
and the `influxd-ctl` command does not include authentication details, the system returns:
...
Use `add-data` instead of the [`join` argument](#join) when [installing a data node](/enterprise_influxdb/v1.11/installation/data_node_installation/) an InfluxDB Enterprise cluster.
...
Resources: [Installation](/enterprise_influxdb/v1.11/installation/data_node_installation/)
...
Use `add-meta` instead of the [`join` argument](#join) when [installing a meta node](/enterprise_influxdb/v1.11/installation/meta_node_installation/) an InfluxDB Enterprise cluster.
...
Creates a backup of a cluster's [metastore](/enterprise_influxdb/v1.11/concepts/glossary/#metastore)
and [shard](/enterprise_influxdb/v1.11/concepts/glossary/#shard) data at that point in time
...
Perform a [full backup](/enterprise_influxdb/v1.11/administration/backup-and-restore/#backup-utility).

###### [ `-rp <rp_name>` ]

Name of the single [retention policy](/enterprise_influxdb/v1.11/concepts/glossary/#retention-policy-rp) to back up (requires the `-db` flag).
...
Resources: [Back up and restore InfluxDB Enterprise](/enterprise_influxdb/v1.11/administration/backup-and-restore/)
...
Copies a [shard](/enterprise_influxdb/v1.11/concepts/glossary/#shard) from a source data node to a destination data node.
...
Resources: [Rebalance InfluxDB Enterprise clusters](/enterprise_influxdb/v1.11/guides/rebalance/)
...
Manage entropy and the [anti-entropy](/enterprise_influxdb/v1.11/administration/anti-entropy/) process among shards in a cluster.
...
4       localhost:8188  1.11.x-c1.11.x  {}
5       localhost:8288  1.11.x-c1.11.x  {}

Meta Nodes
==========
ID      TCP Address     Version       Labels
1       localhost:8191  1.11.x-c1.11.x  {}
2       localhost:8291  1.11.x-c1.11.x  {}
3       localhost:8391  1.11.x-c1.11.x  {}
```
...
==========
ID      TCP Address     Version       Labels
4       localhost:8188  1.11.x-c1.11.x  {"az":"us-east","hello":"earth"}
...
==========
ID      TCP Address     Version       Labels
4       localhost:8188  1.11.x-c1.11.x  {"az":"us-east","hello":"world"}
5       localhost:8288  1.11.x-c1.11.x  {"az":"us-west"}
...
Resources: [Rebalance cluster](/enterprise_influxdb/v1.11/guides/rebalance/)
...
See ["Back up and restore"](/enterprise_influxdb/v1.11/administration/backup-and-restore/#restore-overwrite-metadata-from-a-full-or-incremental-backup-to-fix-damaged-metadata)
...
Integer of the new [replication factor](/enterprise_influxdb/v1.11/concepts/glossary/#replication-factor) to restore to (this is capped to the number of data nodes in the cluster).

###### [ `-newrp <newrp_name>` ]

Name of the new [retention policy](/enterprise_influxdb/v1.11/concepts/glossary/#retention-policy-rp) to restore to.
...
Identifier of the [shard](/enterprise_influxdb/v1.11/concepts/glossary/#shard) to restore.

Resources: [Backing up and restoring in InfluxDB Enterprise](/enterprise_influxdb/v1.11/administration/backup-and-restore/)
...
Shows all [meta nodes](/enterprise_influxdb/v1.11/concepts/glossary/#meta-node) and [data nodes](/enterprise_influxdb/v1.11/concepts/glossary/#data-node) that are part of the cluster. The output includes the `RAFT status` of all meta nodes, the InfluxDB Enterprise version number and any [labels](#node-labels).
...
In this example, the `show` command output displays the cluster includes three meta nodes and two data nodes. Every node is InfluxDB Enterprise version `1.11.0`. The RAFT status shows `metatest_meta_0_1:8089` as the leader node, and the other two meta nodes as peers.
...
4       test_data_0_1:8088    test_data_0_1:8086        1.11.0         {}
5       test_data_0_2:8088    test_data_0_2:8086        1.11.0         {}
 
Meta Nodes
==========
ID      Raft Address            HTTP Address            Version      Labels
1       test_meta_0_1:8089    test_meta_0_1:8091        1.11.0         {}
2       test_meta_1_1:8089    test_meta_1_1:8091        1.11.0         {}
3       test_meta_1_2:8089    test_meta_1_2:8091        1.11.0         {}
...
4	 cluster-node_0_1:8088		  1.11.0          {}	      false
5	 cluster-node_1_1:8088		  1.11.0          {}	      true
6	 cluster-node_2_1:8088		  1.11.0          {}	      false
```

### `show-shards`

Outputs details about existing [shards](/enterprise_influxdb/v1.11/concepts/glossary/#shard) of the cluster, including shard ID, database, [retention policy](/enterprise_influxdb/v1.11/concepts/glossary/#retention-policy-rp), desired replicas, [shard group](/enterprise_influxdb/v1.11/concepts/glossary/#shard-group), starting timestamp, ending timestamp, expiration timestamp, and [data node](/enterprise_influxdb/v1.11/concepts/glossary/#data-node) owners.
...
Updates a data node's address in the [meta store](/enterprise_influxdb/v1.11/concepts/glossary/#meta-service).
...
Truncates hot [shards](/enterprise_influxdb/v1.11/concepts/glossary/#shard), that is, shards that cover the time range that includes the current time ([`now()`](/enterprise_influxdb/v1.11/concepts/glossary/#now)).
...
Determines when to truncate shards after [`now()`](/enterprise_influxdb/v1.11/concepts/glossary/#now).
By default, the tool sets the delay to one minute.
The `duration` is an integer followed by a [duration unit](/enterprise_influxdb/v1.11/query_language/spec/#durations).

Resources: [Cluster rebalancing](/enterprise_influxdb/v1.11/guides/rebalance/)
