...
aliases:
    - /enterprise_influxdb/v1.11/clients/api_client_libraries/
    - /enterprise_influxdb/v1.11/clients/
    - /enterprise_influxdb/v1.11/clients/api
menu:
...
>**Note:** We recommend using the new client libraries on this page to leverage the new read (via Flux) and write APIs and prepare for conversion to InfluxDB 2.0 and InfluxDB Cloud. For more information, see [InfluxDB 2.0 API compatibility endpoints](/enterprise_influxdb/v1.11/tools/api/#influxdb-2-0-api-compatibility-endpoints). Client libraries for [InfluxDB 1.7 and earlier](/influxdb/v1.7/tools/api_client_libraries/) may continue to work, but are not maintained by InfluxData.
