...
aliases:
  ...
  - /enterprise_influxdb/v1/troubleshooting/frequently-asked-questions/
menu:
...
[`SHOW RETENTION POLICIES`](/enterprise_influxdb/v1/query_language/explore-schema/#show-retention-policies)
statement.
...
[configurable](/enterprise_influxdb/v1/administration/config-data-nodes/#check-interval).
...
[configuration](/enterprise_influxdb/v1/administration/configuration/)
settings, writes, queries, and setting the precision in the InfluxDB
[Command Line Interface](/enterprise_influxdb/v1/tools/shell/) (CLI).
...
Check out [CLI/Shell](/enterprise_influxdb/v1/tools/influx-cli/use-influx/) for more useful CLI options.

## How can a non-admin user `USE` a database in the InfluxDB CLI?

In versions prior to v1.3, [non-admin users](/enterprise_influxdb/v1/administration/authentication_and_authorization/#user-types-and-privileges) could not execute a `USE <database_name>` query in the CLI even if they had `READ` and/or `WRITE` permissions on that database.
...
> **Note** that the [`SHOW DATABASES` query](/enterprise_influxdb/v1/query_language/explore-schema/#show-databases) returns only those databases on which the non-admin user has `READ` and/or `WRITE` permissions.
...
You can cancel a long-running interactive query from the CLI using `Ctrl+C`. To stop other long-running query that you see when using the [`SHOW QUERIES`](/influxdb/v1/query_language/spec/#show-queries) command,
you can use the [`KILL QUERY`](/enterprise_influxdb/v1/troubleshooting/query_management/#stop-currently-running-queries-with-kill-query) command to stop it.
...
Timestamps outside that range return a [parsing error](/enterprise_influxdb/v1/troubleshooting/errors/#unable-to-parse-time-outside-range).

## How can I tell what type of data is stored in a field?

The [`SHOW FIELD KEYS`](/enterprise_influxdb/v1/query_language/explore-schema/#show-field-keys) query also returns the field's type.
...
See [Cast Operations](/enterprise_influxdb/v1/query_language/explore-data/#data-types-and-cast-operations)
for an example.
...
Field value types cannot differ within a
[shard](/enterprise_influxdb/v1/concepts/glossary/#shard) but they can differ across
shards.
...
We recommend using InfluxQL's [subqueries](/enterprise_influxdb/v1/query_language/explore-data/#subqueries)
as a workaround.
...
See the
[Data Exploration](/enterprise_influxdb/v1/query_language/explore-data/#subqueries)
...
* [`COUNT()`](/enterprise_influxdb/v1/query_language/functions/#count) with [`DISTINCT()`](/enterprise_influxdb/v1/query_language/functions/#distinct)
* [`CUMULATIVE_SUM()`](/enterprise_influxdb/v1/query_language/functions/#cumulative-sum)
* [`DERIVATIVE()`](/enterprise_influxdb/v1/query_language/functions/#derivative)
* [`DIFFERENCE()`](/enterprise_influxdb/v1/query_language/functions/#difference)
* [`ELAPSED()`](/enterprise_influxdb/v1/query_language/functions/#elapsed)
* [`MOVING_AVERAGE()`](/enterprise_influxdb/v1/query_language/functions/#moving-average)
* [`NON_NEGATIVE_DERIVATIVE()`](/enterprise_influxdb/v1/query_language/functions/#non-negative-derivative)
* [`HOLT_WINTERS()`](/enterprise_influxdb/v1/query_language/functions/#holt-winters) and [`HOLT_WINTERS_WITH_FIT()`](/enterprise_influxdb/v1/query_language/functions/#holt-winters)
...
buckets or to the user-specified [offset interval](/enterprise_influxdb/v1/query_language/explore-data/#advanced-group-by-time-syntax).
...
A query requires at least one [field key](/enterprise_influxdb/v1/concepts/glossary/#field-key) in the `SELECT` clause. If the `SELECT` clause includes only [tag keys](/enterprise_influxdb/v1/concepts/glossary/#tag-key), the query returns an empty response. For more information, see [Data exploration](/enterprise_influxdb/v1/query_language/explore-data/#common-issues-with-the-select-statement).

### SELECT query includes `GROUP BY time()`

If your `SELECT` query includes a [`GROUP BY time()` clause](/enterprise_influxdb/v1/query_language/explore-data/#group-by-time-intervals), only data points between `1677-09-21 00:12:43.145224194` and [`now()`](/enterprise_influxdb/v1/concepts/glossary/#now) are returned. Therefore, if any of your data points occur after `now()`, specify [an alternative upper bound](/enterprise_influxdb/v1/query_language/explore-data/#time-syntax) in your time interval.

(By default, most [`SELECT` queries](/enterprise_influxdb/v1/query_language/explore-data/#the-basic-select-statement) query data with timestamps between `1677-09-21 00:12:43.145224194` and `2262-04-11T23:47:16.854775806Z` UTC.)
...
1. [Launch `influx`](/enterprise_influxdb/v1/tools/influx-cli/use-influx/#launch-influx).
...
For `SELECT` statements with a [`GROUP BY time()` clause](/enterprise_influxdb/v1/query_language/explore-data/#group-by-time-intervals), the default time
...
The function [ELAPSED()](/enterprise_influxdb/v1/query_language/functions/#elapsed)
...
See [Data Exploration](/enterprise_influxdb/v1/query_language/explore-data/#time-syntax) for more on time syntax in queries.
...
This can cause InfluxDB to overwrite [points](/enterprise_influxdb/v1/concepts/glossary/#point) that were previously differentiated by a tag.
...
See the [`TOP()`](/enterprise_influxdb/v1/query_language/functions/#top-tags-and-the-into-clause) and [`BOTTOM()`](/enterprise_influxdb/v1/query_language/functions/#bottom-tags-and-the-into-clause) documentation for more information.
...
InfluxDB is not a relational database and mapping data across measurements is not currently a recommended [schema](/enterprise_influxdb/v1/concepts/glossary/#schema).
...
InfluxDB maintains an in-memory index of every [series](/enterprise_influxdb/v1/concepts/glossary/#series) in the system. As the number of unique series grows, so does the RAM usage. High [series cardinality](/enterprise_influxdb/v1/concepts/glossary/#series-cardinality) can lead to the operating system killing the InfluxDB process with an out of memory (OOM) exception. See [SHOW CARDINALITY](/enterprise_influxdb/v1/query_language/spec/#show-cardinality) to learn about the InfluxSQL commands for series cardinality.
...
[`DROP MEASUREMENT`](/enterprise_influxdb/v1/query_language/manage-database/#delete-measurements-with-drop-measurement), and
[`DROP SERIES`](/enterprise_influxdb/v1/query_language/manage-database/#drop-series-from-the-index-with-drop-series) will all remove series from the index and reduce the overall series cardinality.
...
A point is uniquely identified by the measurement name, [tag set](/enterprise_influxdb/v1/concepts/glossary/#tag-set), and timestamp.
...
This can lead to [non-intuitive errors](/enterprise_influxdb/v1/troubleshooting/errors/#error-parsing-query-found-expected-identifier-at-line-char).
...
First, lengthening your [retention policy](/enterprise_influxdb/v1/concepts/glossary/#retention-policy-rp)‘s [shard group](/enterprise_influxdb/v1/concepts/glossary/#shard-group) duration to cover several years.
...
Increase the shard group duration for your data’s retention policy with the [`ALTER RETENTION POLICY` query](/enterprise_influxdb/v1/query_language/manage-database/#modify-retention-policies-with-alter-retention-policy).

Second, temporarily lowering the [`cache-snapshot-write-cold-duration` configuration setting](/enterprise_influxdb/v1/administration/configure/config-data-nodes/#cache-snapshot-write-cold-duration--10m).
...
[join the meta node to the cluster](/enterprise_influxdb/v1/introduction/installation/installation/meta_node_installation/#step-3-join-the-meta-nodes-to-the-cluster).

## Why am I seeing a `409` error in some of my data node logs?

When you create a
[Continuous Query (CQ)](/enterprise_influxdb/v1/concepts/glossary/#continuous-query-cq)
...
The `_internal` database collects per-node and also cluster-wide information about the InfluxDB Enterprise cluster. The cluster metrics are replicated to other nodes using `consistency=all`. For a [write consistency](/enterprise_influxdb/v1/concepts/clustering/#write-consistency) of `all`, InfluxDB returns a write error (500) for the write attempt even if the points are successfully queued in hinted handoff. Thus, if there are points still in hinted handoff, the `_internal` writes will fail the consistency check and log the error, even though the data is in the durable hinted handoff queue and should eventually persist.
