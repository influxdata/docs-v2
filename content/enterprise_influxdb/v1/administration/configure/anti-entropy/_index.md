...
aliases:
  ...
  - /enterprise_influxdb/v1/administration/anti-entropy/
menu:
...
> [!Warning]
> Prior to InfluxDB Enterprise 1.7.2, the Anti-Entropy (AE) service was enabled  
> by default. When shards create digests with numerous time ranges (tens of  
> thousands), some customers have experienced significant performance issues,  
> including CPU usage spikes. If your shards include a small number of time  
> ranges (most have 1 to 10, some have up to several hundred) and you can  
> benefit from the AE service, enable AE and monitor it closely to determine  
> if your performance is adversely impacted.
...
By default, the service performs consistency checks every 5 minutes. This interval can be modified in the [`anti-entropy.check-interval`](/enterprise_influxdb/v1/administration/config-data-nodes/#check-interval) configuration setting.
...
3. Entropy is reported by the [Anti-Entropy `/status` API](/enterprise_influxdb/v1/administration/anti-entropy-api/#get-status) or with the `influxd-ctl entropy show` command.
...
The configuration settings for the Anti-Entropy service are described in [Anti-Entropy settings](/enterprise_influxdb/v1/administration/configure/config-data-nodes/#anti-entropy-ae-settings) section of the data node configuration.
...
Details on the available API endpoints can be found in [The InfluxDB Anti-Entropy API](/enterprise_influxdb/v1/administration/anti-entropy-api).
...
_View the [Replacing Data Nodes](/enterprise_influxdb/v1/guides/replacing-nodes/#replace-data-nodes-in-an-influxdb-enterprise-cluster) documentation for instructions on replacing data nodes in your InfluxDB Enterprise cluster._
