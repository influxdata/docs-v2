...
Additionally, there is a [`/metrics` endpoint](/influxdb/v1/administration/monitor/performance-metrics-commands/#influxdb-metrics-http-endpoint) configured to produce default Go metrics in Prometheus metrics format.
...
If [authentication is enabled on InfluxDB](/enterprise_influxdb/v1/administration/authentication_and_authorization/),
...
* The Prometheus metric name becomes the InfluxDB [measurement](/enterprise_influxdb/v1/concepts/key_concepts/#measurement) name.
...
* [v1.8.6 and later] Prometheus remote write endpoint drops unsupported Prometheus values (`NaN`,`-Inf`, and `+Inf`) rather than reject the entire batch.
  * If [write trace logging is enabled (`[http] write-tracing = true`)](/enterprise_influxdb/v1/administration/configure/config-data-nodes/#write-tracing), then summaries of dropped values are logged.
