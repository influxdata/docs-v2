...
- To **write to a database using the InfluxDB API**, send a `POST` request to the `/write` endpoint and include the following:
  ...
  For a complete list of the available query parameters, see the [InfluxDB API Reference](/enterprise_influxdb/v1/api/#write-http-endpoint) documentation.
...
- To **write to a database using the InfluxDB 2.0 API (compatible with InfluxDB 1.8+)**, send a `POST` request to the [`/api/v2/write` endpoint](/enterprise_influxdb/v1/api/#api-v2-write-http-endpoint) and include the `?bucket=DATABASE_NAME/RETENTION_POLICY` query parameter--for example:
...
To learn more about using the InfluxDB API, see the [InfluxDB API reference](/enterprise_influxdb/v1/api/#write-http-endpoint).
