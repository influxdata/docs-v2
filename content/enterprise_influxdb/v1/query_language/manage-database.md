...
The `DROP MEASUREMENT` query deletes all data and series from the specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) and deletes the
...
[metastore](/enterprise_influxdb/v1/concepts/glossary/#metastore).
The query takes the following form:
```sql
...
You may disable its auto-creation in the [configuration file](/enterprise_influxdb/v1/administration/configure/config-data-nodes/#retention-autocreate).
...
The `<duration>` is a [duration literal](/enterprise_influxdb/v1/query_language/spec/#durations)
or `INF` (infinite).
...
- Optional. The `SHARD DURATION` clause determines the time range covered by a [shard group](/enterprise_influxdb/v1/concepts/glossary/#shard-group).
- The `<duration>` is a [duration literal](/enterprise_influxdb/v1/query_language/spec/#durations)
...
See [Create a database with CREATE DATABASE](/enterprise_influxdb/v1/query_language/manage-database/#create-database).
