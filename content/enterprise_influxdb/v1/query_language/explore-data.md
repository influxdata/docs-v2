...
See the [Sample Data](/enterprise_influxdb/v1/query_language/data_download/) page to download
...
The data in the `h2o_feet` [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement)
occur at six-minute time intervals.
The measurement has one [tag key](/enterprise_influxdb/v1/concepts/glossary/#tag-key)
(`location`) which has two [tag values](/enterprise_influxdb/v1/concepts/glossary/#tag-value):
`coyote_creek` and `santa_monica`.
The measurement also has two [fields](/enterprise_influxdb/v1/concepts/glossary/#field):
`level description` stores string [field values](/enterprise_influxdb/v1/concepts/glossary/#field-value)
and `water_level` stores float field values.
All of these data is in the `NOAA_water_database` [database](/enterprise_influxdb/v1/concepts/glossary/#database).
...
The `SELECT` statement queries data from a particular [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) or measurements.
...
`SELECT *`
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Returns all [fields](/enterprise_influxdb/v1/concepts/glossary/#field) and [tags](/enterprise_influxdb/v1/concepts/glossary/#tag).
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Returns a specific field and tag.
The `::[field | tag]` syntax specifies the [identifier's](/enterprise_influxdb/v1/concepts/glossary/#identifier) type.
...
The `FROM` clause supports several formats for specifying a [measurement(s)](/enterprise_influxdb/v1/concepts/glossary/#measurement):

`FROM <measurement_name>`
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Returns data from a single measurement.
If you're using the [CLI](/enterprise_influxdb/v1/tools/influx-cli/use-influx/) InfluxDB queries the measurement in the
...
If you're using the [InfluxDB API](/enterprise_influxdb/v1/tools/api/) InfluxDB queries the
measurement in the database specified in the [`db` query string parameter](/enterprise_influxdb/v1/tools/api/#query-string-parameters)
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Returns data from a measurement in a user-specified [database](/enterprise_influxdb/v1/concepts/glossary/#database) and the `DEFAULT`
...
[Identifiers](/enterprise_influxdb/v1/concepts/glossary/#identifier) **must** be double quoted if they contain characters other than `[A-z,0-9,_]`, if they
...
The query selects all [fields](/enterprise_influxdb/v1/concepts/glossary/#field) and
[tags](/enterprise_influxdb/v1/concepts/glossary/#tag) from the `h2o_feet`
[measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).

If you're using the [CLI](/enterprise_influxdb/v1/tools/influx-cli/use-influx/) be sure to enter
...
If you're using the [InfluxDB API](/enterprise_influxdb/v1/tools/api/) be sure to set the
`db` [query string parameter](/enterprise_influxdb/v1/tools/api/#query-string-parameters)
...
[identifier](/enterprise_influxdb/v1/concepts/glossary/#identifier) is a field or tag.
Use `::[field | tag]` to differentiate between [an identical field key and tag key ](/enterprise_influxdb/v1/troubleshooting/frequently-asked-questions/#how-do-i-query-data-with-an-identical-tag-key-and-field-key).
...
See [Mathematical Operators](/enterprise_influxdb/v1/query_language/math_operators/)
...
A query requires at least one [field key](/enterprise_influxdb/v1/concepts/glossary/#field-key)
in the `SELECT` clause to return data.
If the `SELECT` clause only includes a single [tag key](/enterprise_influxdb/v1/concepts/glossary/#tag-key) or several tag keys, the
...
[fields](/enterprise_influxdb/v1/concepts/glossary/#field),
[tags](/enterprise_influxdb/v1/concepts/glossary/#tag), and/or
...
and integer [field values](/enterprise_influxdb/v1/concepts/glossary/#field-value).
...
Single quote [tag values](/enterprise_influxdb/v1/concepts/glossary/#tag-value) in
the `WHERE` clause.
...
For most `SELECT` statements, the default time range is between [`1677-09-21 00:12:43.145224194` and `2262-04-11T23:47:16.854775806Z` UTC](/enterprise_influxdb/v1/troubleshooting/frequently-asked-questions/#what-are-the-minimum-and-maximum-timestamps-that-influxdb-can-store).
...
[measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) with
[field values](/enterprise_influxdb/v1/concepts/glossary/#field-value) of `water_level`
...
[tag key](/enterprise_influxdb/v1/concepts/glossary/#tag-key) `location` is set to `santa_monica`.
...
or string [field values](/enterprise_influxdb/v1/concepts/glossary/#field-value).
...
`GROUP BY <tag>` groups query results by one or more specified [tags](/enterprise_influxdb/v1/concepts/glossary/#tag).
...
`GROUP BY *`
&emsp;&emsp;&emsp;Groups results by all [tags](/enterprise_influxdb/v1/concepts/glossary/#tag)
...
The order of the [tag keys](/enterprise_influxdb/v1/concepts/glossary/#tag-key) is irrelevant.
...
The query uses an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/)
to calculate the average `water_level` for each
[tag value](/enterprise_influxdb/v1/concepts/glossary/#tag-value) of `location` in
...
The query uses an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/) to calculate the average `index` for
each combination of the `location` [tag](/enterprise_influxdb/v1/concepts/glossary/#tag) and the `randtag` tag in the
...
[tag](/enterprise_influxdb/v1/concepts/glossary/#tag) combination in the `h2o_quality`
measurement.
...
Basic `GROUP BY time()` queries require an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/)
...
[duration literal](/enterprise_influxdb/v1/query_language/spec/#durations).
...
The query uses an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/)
to count the number of `water_level` points with the [tag](/enterprise_influxdb/v1/concepts/glossary/#tag)
`location = coyote_creek` and it group results into 12 minute intervals.

The result for each [timestamp](/enterprise_influxdb/v1/concepts/glossary/#timestamp)
...
The query returns two [series](/enterprise_influxdb/v1/concepts/glossary/#series) of results: one for each
[tag value](/enterprise_influxdb/v1/concepts/glossary/#tag-value) of the `location` tag.
...
Advanced `GROUP BY time()` queries require an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/)
in the [`SELECT` clause](#the-basic-select-statement) and a time range in the
[`WHERE` clause](#the-where-clause).
Note that the `GROUP BY` clause must come after the `WHERE` clause.
...
`GROUP(ing) BY` several things (for example, both [tags](/enterprise_influxdb/v1/concepts/glossary/#tag) and a time interval).
...
The `INTO` clause writes query results to a user-specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
The `INTO` clause supports several formats for specifying a [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement):

`INTO <measurement_name>`
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Writes data to the specified measurement.
If you're using the [CLI](/enterprise_influxdb/v1/tools/influx-cli/use-influx/) InfluxDB writes the data to the measurement in the
[`USE`d](/enterprise_influxdb/v1/tools/influx-cli/use-influx/#commands)
[database](/enterprise_influxdb/v1/concepts/glossary/#database) and the `DEFAULT` [retention policy](/enterprise_influxdb/v1/concepts/glossary/#retention-policy-rp).
If you're using the [InfluxDB API](/enterprise_influxdb/v1/tools/api/) InfluxDB writes the data to the
measurement in the database specified in the [`db` query string parameter](/enterprise_influxdb/v1/tools/api/#query-string-parameters)
and the `DEFAULT` retention policy.
...
See [Database Management](/enterprise_influxdb/v1/query_language/manage-database/)
...
The query writes its results a new [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement): `h2o_feet_copy_1`.
If you're using the [CLI](/enterprise_influxdb/v1/tools/influx-cli/use-influx/), InfluxDB writes the data to
...
If you're using the [InfluxDB API](/enterprise_influxdb/v1/tools/api/), InfluxDB writes the
data to the database and retention policy specified in the `db` and `rp`
[query string parameters](/enterprise_influxdb/v1/tools/api/#query-string-parameters).
If you do not set the `rp` query string parameter, the InfluxDB API automatically
...
InfluxQL [function](/enterprise_influxdb/v1/query_language/functions) and a [`GROUP BY
...
If an `INTO` query includes a [tag key](/enterprise_influxdb/v1/concepts/glossary#tag-key) in the [`SELECT` clause](#the-basic-select-statement), the query converts [tags](/enterprise_influxdb/v1/concepts/glossary#tag) in the current
measurement to [fields](/enterprise_influxdb/v1/concepts/glossary#field) in the destination measurement.
This can cause InfluxDB to overwrite [points](/enterprise_influxdb/v1/concepts/glossary#point) that were previously differentiated
by a [tag value](/enterprise_influxdb/v1/concepts/glossary#tag-value).
Note that this behavior does not apply to queries that use the [`TOP()`](/enterprise_influxdb/v1/query_language/functions/#top) or [`BOTTOM()`](/enterprise_influxdb/v1/query_language/functions/#bottom) functions.
The
[Frequently Asked Questions](/enterprise_influxdb/v1/troubleshooting/frequently-asked-questions/#why-are-my-into-queries-missing-data)
...
See the [Continuous Queries](/enterprise_influxdb/v1/query_language/continuous_queries/)
documentation for how to automate `INTO` clause queries on realtime data.
Among [other uses](/enterprise_influxdb/v1/query_language/continuous_queries/#continuous-query-use-cases),
Continuous Queries automate the downsampling process.

## ORDER BY time DESC

By default, InfluxDB returns results in ascending time order; the first [point](/enterprise_influxdb/v1/concepts/glossary/#point)
returned has the oldest [timestamp](/enterprise_influxdb/v1/concepts/glossary/#timestamp) and
...
`h2o_feet` [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) first.
...
[points](/enterprise_influxdb/v1/concepts/glossary/#point) and the number of
[series](/enterprise_influxdb/v1/concepts/glossary/#series) returned per query.

## The LIMIT clause

`LIMIT <N>` returns the first `N` [points](/enterprise_influxdb/v1/concepts/glossary/#point) from the specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
`N` specifies the number of [points](/enterprise_influxdb/v1/concepts/glossary/#point) to return from the specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
The query returns the three oldest [points](/enterprise_influxdb/v1/concepts/glossary/#point) (determined by timestamp) from the `h2o_feet` [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
to calculate the average `water_level` for each [tag](/enterprise_influxdb/v1/concepts/glossary/#tag) and for each twelve-minute
interval in the query's time range.
`LIMIT 2` requests the two oldest twelve-minute averages (determined by timestamp).

Note that without `LIMIT 2`, the query would return four points per [series](/enterprise_influxdb/v1/concepts/glossary/#series);
one for each twelve-minute interval in the query's time range.

## The `SLIMIT` clause

`SLIMIT <N>` returns every [point](/enterprise_influxdb/v1/concepts/glossary/#point) from \<N> [series](/enterprise_influxdb/v1/concepts/glossary/#series) in the specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
`N` specifies the number of [series](/enterprise_influxdb/v1/concepts/glossary/#series) to return from the specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
The query returns all `water_level` [points](/enterprise_influxdb/v1/concepts/glossary/#point) from one of the [series](/enterprise_influxdb/v1/concepts/glossary/#series) associated
with the `h2o_feet` [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
`LIMIT <N>` followed by `SLIMIT <N>` returns the first \<N> [points](/enterprise_influxdb/v1/concepts/glossary/#point) from \<N> [series](/enterprise_influxdb/v1/concepts/glossary/#series) in the specified measurement.
...
`N1` specifies the number of [points](/enterprise_influxdb/v1/concepts/glossary/#point) to return per [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
If `N1` is greater than the number of points in a measurement, InfluxDB returns all points from that measurement.

`N2` specifies the number of series to return from the specified [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
The query returns the three oldest [points](/enterprise_influxdb/v1/concepts/glossary/#point) (determined by timestamp) from one
of the [series](/enterprise_influxdb/v1/concepts/glossary/#series) associated with the
[measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) `h2o_feet`.
...
`OFFSET` and `SOFFSET` paginates [points](/enterprise_influxdb/v1/concepts/glossary/#point) and [series](/enterprise_influxdb/v1/concepts/glossary/#series) returned.
...
`OFFSET <N>` paginates `N` [points](/enterprise_influxdb/v1/concepts/glossary/#point) in the query results.
...
`N` specifies the number of [points](/enterprise_influxdb/v1/concepts/glossary/#point) to paginate.
...
The query returns the fourth, fifth, and sixth [points](/enterprise_influxdb/v1/concepts/glossary/#point) from the `h2o_feet` [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement).
...
`SOFFSET <N>` paginates `N` [series](/enterprise_influxdb/v1/concepts/glossary/#series) in the query results.
...
`N` specifies the number of [series](/enterprise_influxdb/v1/concepts/glossary/#series) to paginate.
...
The query returns data for the [series](/enterprise_influxdb/v1/concepts/glossary/#series) associated with the `h2o_feet`
[measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) and the `location = santa_monica` [tag](/enterprise_influxdb/v1/concepts/glossary/#tag).
...
The returned timestamps must be in [RFC3339 format](/enterprise_influxdb/v1/query_language/explore-data/#configuring-the-returned-timestamps) for the UTC offset or UTC DST to appear.
...
the default time range is between `1677-09-21 00:12:43.145224194` UTC and [`now()`](/enterprise_influxdb/v1/concepts/glossary/#now).
...
Include a [duration literal](/enterprise_influxdb/v1/query_language/spec/#durations)
...
Add (`+`) or subtract (`-`) a time from a timestamp with a [duration literal](/enterprise_influxdb/v1/query_language/spec/#durations).
...
The query returns data with timestamps that occur between August 18, 2015
at 00:00:00 and August 18, 2015 at 00:12:00.
The `s` [duration literal](/enterprise_influxdb/v1/query_language/spec/#durations) at the
...
Use [`now()`](/enterprise_influxdb/v1/concepts/glossary/#now) to query data with [timestamps](/enterprise_influxdb/v1/concepts/glossary/#timestamp) relative to the server's current timestamp.
...
For more information, see [Frequently asked questions](/enterprise_influxdb/v1/troubleshooting/frequently-asked-questions/#why-is-my-query-with-a-where-or-time-clause-returning-empty-results).
...
The [CLI](/enterprise_influxdb/v1/tools/influx-cli/use-influx/) returns timestamps in
nanosecond epoch format by default.
Specify alternative formats with the
[`precision <format>` command](/enterprise_influxdb/v1/tools/influx-cli/use-influx/#influx-commands).
The [InfluxDB API](/enterprise_influxdb/v1/tools/api/) returns timestamps
in [RFC3339](https://www.ietf.org/rfc/rfc3339.txt) format by default.
Specify alternative formats with the
[`epoch` query string parameter](/enterprise_influxdb/v1/tools/api/#query-string-parameters).
...
The query selects all [field keys](/enterprise_influxdb/v1/concepts/glossary/#field-key)
and [tag keys](/enterprise_influxdb/v1/concepts/glossary/#tag-key) that include an `l`.
...
The query uses an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/)
to calculate the average `degrees` for every [measurement](/enterprise_influxdb/v1/concepts/glossary#measurement) in the `NOAA_water_database`
[database](/enterprise_influxdb/v1/concepts/glossary#database) that contains the word `temperature`.
...
The query uses an InfluxQL [function](/enterprise_influxdb/v1/query_language/functions/)
to calculate the average `water_level` where the [tag value](/enterprise_influxdb/v1/concepts/glossary#tag-value) of `location`
...
[tag](/enterprise_influxdb/v1/concepts/glossary#tag) has no value.
Every data [point](/enterprise_influxdb/v1/concepts/glossary#point) in the `NOAA_water_database` has a tag value for `location`.

It's possible to perform this same query without a regular expression.
See the
[Frequently Asked Questions](/enterprise_influxdb/v1/troubleshooting/frequently-asked-questions/#how-do-i-select-data-with-a-tag-that-has-no-value)
...
The [`SELECT` clause](#the-basic-select-statement) supports specifying a [field's](/enterprise_influxdb/v1/concepts/glossary/#field) type and basic cast
...
Currently, InfluxDB supports casting [field values](/enterprise_influxdb/v1/concepts/glossary/#field-value) from integers to
...
The query returns the integer form of `water_level`'s float [field values](/enterprise_influxdb/v1/concepts/glossary/#field-value).
...
In InfluxDB, queries merge [series](/enterprise_influxdb/v1/concepts/glossary/#series)
automatically.

### Example

The `h2o_feet` [measurement](/enterprise_influxdb/v1/concepts/glossary/#measurement) in the `NOAA_water_database` is part of two [series](/enterprise_influxdb/v1/concepts/glossary/#series).
The first series is made up of the `h2o_feet` measurement and the `location = coyote_creek` [tag](/enterprise_influxdb/v1/concepts/glossary/#tag).
The second series is made of up the `h2o_feet` measurement and the `location = santa_monica` tag.

The following query automatically merges those two series when it calculates the [average](/enterprise_influxdb/v1/query_language/functions/#mean) `water_level`:
...
In the InfluxDB [CLI](/enterprise_influxdb/v1/tools/influx-cli/use-influx/):
...
With the [InfluxDB API](/enterprise_influxdb/v1/tools/api/):
...
#### Calculate the [`SUM()`](/enterprise_influxdb/v1/query_language/functions/#sum) of several [`MAX()`](/enterprise_influxdb/v1/query_language/functions/#max) values
...
#### Calculate the [`MEAN()`](/enterprise_influxdb/v1/query_language/functions/#mean) difference between two fields
...
#### Calculate several [`MEAN()`](/enterprise_influxdb/v1/query_language/functions/#mean) values and place a condition on those mean values
...
#### Calculate the [`SUM()`](/enterprise_influxdb/v1/query_language/functions/#sum) of several [`DERIVATIVE()`](/enterprise_influxdb/v1/query_language/functions/#derivative) values
