...
menu:
  enterprise_influxdb_1_10:
    ...
    parent: InfluxQL
aliases:
  - /enterprise_influxdb/v1.10/query_language/database_management/
---
...
If you're looking for `SHOW` queries (for example, `SHOW DATABASES` or `SHOW RETENTION POLICIES`), see [Schema Exploration](/enterprise_influxdb/v1.10/query_language/explore-schema).

The examples in the sections below use the InfluxDB [Command Line Interface (CLI)](/enterprise_influxdb/v1.10/introduction/getting-started/).
...
For more on using the InfluxDB API, see [Querying data](/enterprise_influxdb/v1.10/guides/querying_data/).

> **Note:** When authentication is enabled, only admin users can execute most of the commands listed on this page.
> See the documentation on [authentication and authorization](/enterprise_influxdb/v1.10/administration/authentication_and_authorization/) for more information.
...
`CREATE DATABASE` requires a database [name](/enterprise_influxdb/v1.10/troubleshooting/frequently-asked-questions/#what-words-and-characters-should-i-avoid-when-writing-data-to-influxdb).

The `WITH`, `DURATION`, `REPLICATION`, `SHARD DURATION`, and `NAME` clauses are optional and create a single [retention policy](/enterprise_influxdb/v1.10/concepts/glossary/#retention-policy-rp) associated with the created database.
...
For more information about those clauses, see [Retention Policy Management](/enterprise_influxdb/v1.10/query_language/manage-database/#retention-policy-management).
...
[By default](/enterprise_influxdb/v1.10/administration/configure/config-data-nodes/#retention-autocreate), InfluxDB also creates the `autogen` retention policy and associates it with the `NOAA_water_database`.
...
The query creates a database called `NOAA_water_database`.
It also creates a default retention policy for `NOAA_water_database` with a `DURATION` of three days, a [replication factor](/enterprise_influxdb/v1.10/concepts/glossary/#replication-factor) of one, a [shard group](/enterprise_influxdb/v1.10/concepts/glossary/#shard-group) duration of one hour, and with the name `liquid`.
...
The `DROP SERIES` query deletes all points from a [series](/enterprise_influxdb/v1.10/concepts/glossary/#series) in a database,
...
[series](/enterprise_influxdb/v1.10/concepts/glossary/#series) in a database.
Unlike
[`DROP SERIES`](/enterprise_influxdb/v1.10/query_language/manage-database/#drop-series-from-the-index-with-drop-series), `DELETE` does not drop the series from the index.
...
* `DELETE` supports
[regular expressions](/enterprise_influxdb/v1.10/query_language/explore-data/#regular-expressions)
in the `FROM` clause when specifying measurement names and in the `WHERE` clause
when specifying tag values.
* `DELETE` does not support [fields](/enterprise_influxdb/v1.10/concepts/glossary/#field) in the `WHERE` clause.
...
The `DROP MEASUREMENT` query deletes all data and series from the specified [measurement](/enterprise_influxdb/v1.10/concepts/glossary/#measurement) and deletes the
...
[metastore](/enterprise_influxdb/v1.10/concepts/glossary/#metastore).
The query takes the following form:
```sql
...
You may disable its auto-creation in the [configuration file](/enterprise_influxdb/v1.10/administration/configure/config-data-nodes/#retention-autocreate).
...
The `<duration>` is a [duration literal](/enterprise_influxdb/v1.10/query_language/spec/#durations)
or `INF` (infinite).
...
- Optional. The `SHARD DURATION` clause determines the time range covered by a [shard group](/enterprise_influxdb/v1.10/concepts/glossary/#shard-group).
- The `<duration>` is a [duration literal](/enterprise_influxdb/v1.10/query_language/spec/#durations)
...
See
[Shard group duration management](/enterprise_influxdb/v1.10/concepts/schema_and_data_layout/#shard-group-duration-management)
...
See [Create a database with CREATE DATABASE](/enterprise_influxdb/v1.10/query_language/manage-database/#create-database).
