...
[function](/enterprise_influxdb/v1.10/concepts/glossary/#function),
an [`INTO` clause](/enterprise_influxdb/v1.10/query_language/spec/#clauses),
and a [`GROUP BY time()` clause](/enterprise_influxdb/v1.10/query_language/spec/#clauses):
...
[`now()`](/enterprise_influxdb/v1.10/concepts/glossary/#now) and `now()` minus the
...
[Fully qualify](/enterprise_influxdb/v1.10/query_language/explore-data/#the-basic-select-statement)
the destination measurement to store the downsampled data in a non-`DEFAULT`
[retention policy](/enterprise_influxdb/v1.10/concepts/glossary/#retention-policy-rp) (RP).
...
See the [Downsampling and Data Retention](/enterprise_influxdb/v1.10/guides/downsampling_and_retention/)
...
[backreferencing syntax](/enterprise_influxdb/v1.10/query_language/explore-data/#the-into-clause)
...
[`fill()`](/enterprise_influxdb/v1.10/query_language/explore-data/#group-by-time-intervals-and-fill)
...
relative to [`now()`](/enterprise_influxdb/v1.10/concepts/glossary/#now).
Use a basic
[`INTO` query](/enterprise_influxdb/v1.10/query_language/explore-data/#the-into-clause)
to backfill results for data with older timestamps.

##### Missing tags in the CQ results

By default, all
[`INTO` queries](/enterprise_influxdb/v1.10/query_language/explore-data/#the-into-clause)
...
See [ Description of Basic Syntax](/enterprise_influxdb/v1.10/query_language/continuous_queries/#description-of-basic-syntax).
...
When the CQ executes, it runs a single query for the time range between
[`now()`](/enterprise_influxdb/v1.10/concepts/glossary/#now) and `now()` minus the `FOR` interval in the `RESAMPLE` clause.
...
[duration literals](/enterprise_influxdb/v1.10/query_language/spec/#durations).
The `RESAMPLE` clause works with either or both of the `EVERY` and `FOR` intervals
configured.
CQs default to the relevant
[basic syntax behavior](/enterprise_influxdb/v1.10/query_language/continuous_queries/#description-of-basic-syntax)
if the `EVERY` interval or `FOR` interval is not provided (see the first issue in
[Common Issues with Advanced Syntax](/enterprise_influxdb/v1.10/query_language/continuous_queries/#common-issues-with-advanced-syntax)
...
[handles duplicate points](/enterprise_influxdb/v1.10/troubleshooting/frequently-asked-questions/#how-does-influxdb-handle-duplicate-points)
...
See [Frequently Asked Questions](/enterprise_influxdb/v1.10/troubleshooting/frequently-asked-questions/#why-does-fill-previous-return-empty-results)
...
Only admin users are allowed to work with CQs. For more on user privileges, see [Authentication and Authorization](/enterprise_influxdb/v1.10/administration/authentication_and_authorization/#user-types-and-privileges).
...
Information about CQ configuration settings is available in the [Configuration](/enterprise_influxdb/v1.10/administration/configure/config-data-nodes/#continuous-queries-settings) documentation.
...
[retention policies](/enterprise_influxdb/v1.10/concepts/glossary/#retention-policy-rp)
...
See the
[Downsampling and data retention](/enterprise_influxdb/v1.10/guides/downsampling_and_retention/)
...
> **Note:** InfluxQL supports [subqueries](/enterprise_influxdb/v1.10/query_language/explore-data/#subqueries) which also offer similar functionality to `HAVING` clauses.
See [Data Exploration](/enterprise_influxdb/v1.10/query_language/explore-data/#subqueries) for more information.
...
[support nesting](/enterprise_influxdb/v1.10/troubleshooting/frequently-asked-questions/#which-influxql-functions-support-nesting)
of other functions.
Most do not.
...
> **Note:** InfluxQL supports [subqueries](/enterprise_influxdb/v1.10/query_language/explore-data/#subqueries) which also offer the same functionality as nested functions.
See [Data Exploration](/enterprise_influxdb/v1.10/query_language/explore-data/#subqueries) for more information.
...
precision data, see [Downsampling and data retention](/enterprise_influxdb/v1.10/guides/downsampling_and_retention/).
