...
not specify a [database](/enterprise_influxdb/v1.10/concepts/glossary/#database).
Specify a database with an `ON` clause in the `SHOW` query, with `USE <database_name>` in the
[CLI](/enterprise_influxdb/v1.10/tools/influx-cli/use-influx/), or with the `db` query string parameter in
the [InfluxDB API](/enterprise_influxdb/v1.10/tools/api/#query-string-parameters) request.
...
**Resources:**
[Schema exploration](/enterprise_influxdb/v1.10/query_language/explore-schema/),
[InfluxQL reference](/enterprise_influxdb/v1.10/query_language/spec/)

## `error: max series per database exceeded: < >`

The `max series per database exceeded` error occurs when a write causes the
number of [series](/enterprise_influxdb/v1.10/concepts/glossary/#series) in a database to
exceed the maximum allowable series per database.
The maximum allowable series per database is controlled by the
[`max-series-per-database`](/enterprise_influxdb/v1.10/administration/configure/config-data-nodes/#max-series-per-database--1000000)
...
[identifiers](/enterprise_influxdb/v1.10/concepts/glossary/#identifier) in the query is an
[InfluxQL Keyword](/enterprise_influxdb/v1.10/query_language/spec/#keywords).
...
[Query Language Documentation](/enterprise_influxdb/v1.10/query_language/)
...
includes both an [aggregate function](/enterprise_influxdb/v1.10/query_language/functions/)
and a standalone [field key](/enterprise_influxdb/v1.10/concepts/glossary/#field-key) or
...
**Resources:**
[Data Exploration](/enterprise_influxdb/v1.10/query_language/explore-data/#time-syntax)
...
[line protocol](/enterprise_influxdb/v1.10/concepts/glossary/#influxdb-line-protocol) includes a
...
**Resources:**
[InfluxDB line protocol tutorial](/enterprise_influxdb/v1.10/write_protocols/line_protocol_tutorial/),
[InfluxDB line protocol reference](/enterprise_influxdb/v1.10/write_protocols/line_protocol_reference/)

## `unable to parse < >: time outside range`

The `time outside range` error occurs when the timestamp in the
[InfluxDB line protocol](/enterprise_influxdb/v1.10/concepts/glossary/#influxdb-line-protocol)
...
**Resources:**
[InfluxDB line protocol tutorial](/enterprise_influxdb/v1.10/write_protocols/line_protocol_tutorial/#data-types),
[InfluxDB line protocol reference](/enterprise_influxdb/v1.10/write_protocols/line_protocol_reference/#data-types)
...
[`cache-max-memory-size` setting](/enterprise_influxdb/v1.10/administration/configure/config-data-nodes/#cache-max-memory-size--1g)
...
or for datasets with higher [series cardinality](/enterprise_influxdb/v1.10/concepts/glossary/#series-cardinality).
...
[`_internal` database](/enterprise_influxdb/v1.10/administration/monitor/server_monitoring/#internal-monitoring)
...
**Resources:**
[Query management](/enterprise_influxdb/v1.10/troubleshooting/query_management/)
...
2. Review import errors for possible causes to resolve:
  ...
  >**Note:** To learn how to use the `-import` command, see [Import data from a file with `-import`](/enterprise_influxdb/v1.10/tools/influx-cli/use-influx/#import-data-from-a-file-with-import).
