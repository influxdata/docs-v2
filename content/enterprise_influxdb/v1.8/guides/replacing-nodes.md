...
**Meta nodes** manage and monitor both the uptime of nodes in the cluster and distribution of [shards](/influxdb/2.0/concepts/glossary/#shard) among nodes in the cluster. Meta nodes hold information about which data nodes own which shards; information that the [anti-entropy](/enterprise_influxdb/v1.8/administration/anti-entropy/) (AE) process depends on.
...
If replacing a meta node that is either unreachable or unrecoverable, you must forcefully remove the node from the meta cluster. See [step 2.2](#2-2-remove-the-non-leader-meta-node) of the [replace meta nodes](#replacr-meta-nodes-in-an-influxdb-enterprise-cluster) process.
...
To resolve this, log in to a meta node and use the [`influxd-ctl update-data`](/enterprise_influxdb/v1.8/administration/cluster-commands/#update-data) command
...
[Provision and start a new meta node](/enterprise_influxdb/v1.8/production_installation/meta_node_installation/), but **do not** add it to the cluster yet.
...
[Provision and start a new data node](/enterprise_influxdb/v1.8/production_installation/data_node_installation/), but **do not** add it to your cluster yet.
...
Within the duration defined by [`anti-entropy.check-interval`](/enterprise_influxdb/v1.8/administration/config-data-nodes#check-interval-10m),
