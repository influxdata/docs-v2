...
2. Enter your query (see [Explore data with Flux and the Data Explorer](/influxdb/version/visualize-data/explore-metrics/#explore-data-with-flux-and-the-data-explorer)). You must include the aggregate functions used to determine the Band Plot visualization boundaries in your query.
...
4. Create three aggregate functions: one for the main boundary, one for the upper boundary, and one for the lower boundary. The following example uses the [`mean()`](/flux/v0/stdlib/universe/mean/), [`max()`](/flux/v0/stdlib/universe/max/), and [`min()`](/flux/v0/stdlib/universe/min) functions:
...
5. (Optional) Customize the name of the yielded results for each function by editing the `name` parameter in the [`yield()`](/flux/v0/stdlib/universe/yield/) function.
