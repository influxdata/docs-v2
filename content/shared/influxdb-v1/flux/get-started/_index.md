...
The `influx` CLI also includes a `-type=flux` option that starts an interactive
Flux Read-Eval-Print-Loop (REPL), which lets you write and run Flux queries from
the command line.

---

_For information about downloading and installing InfluxDB, see [InfluxDB installation](/product/version/introduction/installation)._
...
You have multiple [options for writing and running Flux queries](/product/version/flux/guides/execute-queries/),
...
Use the `-type=flux` and `-path-prefix=/api/v2/query` options to open a Flux REPL
where you write and run Flux queries.
For the required InfluxDB version and the authentication options, see
[Execute Flux queries](/product/version/flux/guides/execute-queries/).
...
<div class="page-nav-btns">
  <a class="btn next" href="/product/version/flux/get-started/query-influxdb/">Query InfluxDB with Flux</a>
</div>
