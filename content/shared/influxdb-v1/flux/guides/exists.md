...
{{% note %}}
Static record types are enforced by the Flux version bundled with InfluxDB.
InfluxDB v1.11+ returns the error above.
InfluxDB v1.8 bundles Flux v0.65, which accepts `exists` on a key that the
record literal does not define.
{{% /note %}}
