...
1. For the time series use case, we assume that if the same data is sent multiple times, it is the exact same data that a client just sent several times.
    ...
    _**Pro:**_ Simplified [conflict resolution](/product/version/troubleshooting/frequently-asked-questions/#how-does-influxdb-handle-duplicate-points) increases write performance.  
...
7. Many time [series](/product/version/concepts/glossary/#series) are ephemeral.
    There are often time series that appear only for a few hours and then go away, for example.
    ...
    _**Con:**_ Schema-less design means that some database functions are not supported, for example, there are no cross table joins.
