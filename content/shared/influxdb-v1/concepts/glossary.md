...
see [InfluxQL functions](/product/version/query_language/functions/#aggregations).
...
A bucket is a named location where time series data is stored in **InfluxDB 2.0**. In InfluxDB 1.8+, each combination of a database and a retention policy (database/retention-policy) represents a bucket. Use the [InfluxDB 2.x API compatibility endpoints](/product/version/tools/api#influxdb-2-0-api-compatibility-endpoints) included with InfluxDB 1.8+ to interact with buckets.
...
See [Continuous Queries](/product/version/query_language/continuous_queries/).
...
See [Database Management](/product/version/query_language/manage-database/#create-retention-policies-with-create-retention-policy) for how to set duration.
...
See [InfluxQL Functions](/product/version/query_language/functions/) for a complete list of InfluxQL functions.
...
user names.
See [Query Language Specification](/product/version/query_language/spec/#identifiers).
...
The text based format for writing points to InfluxDB. See [InfluxDB line protocol](/product/version/write_protocols/).
...
For more information about duplicate points, see [How does InfluxDB handle duplicate points?](/product/version/troubleshooting/frequently-asked-questions/#how-does-influxdb-handle-duplicate-points)
...
See [Data Exploration](/product/version/query_language/explore-data/), [Schema Exploration](/product/version/query_language/explore-schema/), [Database Management](/product/version/query_language/manage-database/).

## replication factor (RF)

{{% show-in "influxdb/v1" %}}
The attribute of the retention policy that determines how many copies of data to
concurrently store (or retain) in a cluster.
Replicating copies ensures that data is available when one or more data nodes are
unavailable.

Replication requires multiple data nodes, which are available only in
[InfluxDB Enterprise](/enterprise_influxdb/v1/) clusters.
InfluxDB OSS runs as a single node, so the replication factor has no effect --
there is only ever one copy of your data.
For default replication factor behavior in a cluster, see the InfluxDB Enterprise
[`CREATE RETENTION POLICY` reference](/enterprise_influxdb/v1/query_language/manage-database/#replication).
{{% /show-in %}}

{{% show-in "enterprise_influxdb/v1" %}}
The attribute of the retention policy that determines how many copies of your
data the cluster stores.
Replicating copies ensures that data is accessible when one or more data nodes
are unavailable.
...
For more information, see [Retention policy management](/product/version/query_language/manage-database/#retention-policy-management).
...
How the data is organized in InfluxDB.
...
See [Schema Design](/product/version/concepts/schema_and_data_layout/) for more information.
...
See [InfluxQL Functions](/product/version/query_language/functions/#selectors) for a complete list of the available and upcoming selectors.
...
See [SHOW CARDINALITY](/product/version/query_language/spec/#show-cardinality) to learn about the InfluxQL commands for series cardinality.
...
See [Retention Policy management](/product/version/query_language/manage-database/#retention-policy-management) for more information.
...
For how to specify time when writing data, see [Write Syntax](/product/version/write_protocols/write_syntax/).
For how to specify time when querying data, see [Data Exploration](/product/version/query_language/explore-data/#time-syntax).
...
See [InfluxQL Functions](/product/version/query_language/functions/#transformations) for a complete list of the available and upcoming aggregations.

Related entries: [aggregation](#aggregation), [function](#function), [selector](#selector)

## TSM (Time Structured Merge tree)

The purpose-built data storage format for InfluxDB. TSM allows for greater compaction and higher write and read throughput than existing B+ or LSM tree implementations. See [Storage Engine](/product/version/concepts/storage_engine/) for more.
...
See [Authentication and Authorization](/product/version/administration/authentication_and_authorization/).
