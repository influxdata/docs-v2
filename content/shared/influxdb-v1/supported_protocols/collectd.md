...
If you're running Linux or FreeBSD, please adjust your operating system UDP buffer size limit, [see here for more details.](/product/version/supported_protocols/udp/#a-note-on-udp-ip-os-buffer-sizes)
...
Multi-value plugins can be handled two ways. Setting parse-multivalue-plugin to "split" will parse and store the multi-value plugin data (for example, df free:5000,used:1000) into separate measurements (for example, (df_free, value=5000) (df_used, value=1000)), while "join" will parse and store the multi-value plugin as a single multi-value measurement (for example, (df, free=5000,used=1000)). "split" is the default behavior for backward compatibility with previous versions of influxdb.
